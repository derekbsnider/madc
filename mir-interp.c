...
static ALWAYS_INLINE uint8_t get_v128_u8 (const uint8_t *v, size_t ind) { return v[ind]; }

static ALWAYS_INLINE void set_v128_u8 (uint8_t *v, size_t ind, uint8_t val) { v[ind] = val; }

static ALWAYS_INLINE uint16_t get_v128_u16 (const uint8_t *v, size_t ind) {
  uint16_t val;

  memcpy (&val, v + ind * sizeof (uint16_t), sizeof (val));
  return val;
}

static ALWAYS_INLINE void set_v128_u16 (uint8_t *v, size_t ind, uint16_t val) {
  memcpy (v + ind * sizeof (uint16_t), &val, sizeof (val));
}
...
static ALWAYS_INLINE float get_v128_f32 (const uint8_t *v, size_t ind) {
  ...
  memcpy (&val, v + ind * sizeof (float), sizeof (val));
  return val;
}
...
#define REP_SEP ;
  if (bp == NULL) {
    ...
    REP8 (LAB_EL, MIR_MUL, MIR_MULS, MIR_FMUL, MIR_DMUL, MIR_LDMUL, MIR_VMULI16, MIR_VMULF32,
          MIR_VMULF64);
    LAB_EL (MIR_DIV);
    ...
    REP6 (LAB_EL, MIR_VLSHI16, MIR_VRSHI16, MIR_VURSHI16, MIR_VLSHI32, MIR_VRSHI32,
          MIR_VURSHI32);
...
common_addr:;
  ...
  CASE (MIR_VLSHI16, 3) {
    ...
    for (size_t i = 0; i < 8; i++) set_v128_u16 (r, i, get_v128_u16 (op1, i) << count);
    END_INSN;
  }
  CASE (MIR_VRSHI16, 3) {
    uint8_t *r = get_vop (bp, ops);
    ...
    for (size_t i = 0; i < 8; i++) set_v128_u16 (r, i, (int16_t) get_v128_u16 (op1, i) >> count);
    END_INSN;
  }
  CASE (MIR_VURSHI16, 3) {
    uint8_t *r = get_vop (bp, ops);
  ...
  CASE (MIR_VLSHI32, 3) {
    ...
    for (size_t i = 0; i < 4; i++) set_v128_u32 (r, i, get_v128_u32 (op1, i) << count);
    END_INSN;
  }
  CASE (MIR_VRSHI32, 3) {
    uint8_t *r = get_vop (bp, ops);
    ...
    for (size_t i = 0; i < 4; i++) set_v128_u32 (r, i, (int32_t) get_v128_u32 (op1, i) >> count);
    END_INSN;
  }
  CASE (MIR_VURSHI32, 3) {
    uint8_t *r = get_vop (bp, ops);
  ...
  CASE (MIR_VEQI8, 3) {
    ...
    for (size_t i = 0; i < 16; i++)
      set_v128_u8 (r, i, get_v128_u8 (op1, i) == get_v128_u8 (op2, i) ? UINT8_MAX : 0);
    END_INSN;
  }
  CASE (MIR_VEQI16, 3) {
    uint8_t *r = get_vop (bp, ops);
    ...
    for (size_t i = 0; i < 8; i++)
      set_v128_u16 (r, i, get_v128_u16 (op1, i) == get_v128_u16 (op2, i) ? UINT16_MAX : 0);
    END_INSN;
  }
  CASE (MIR_VEQI32, 3) {
    uint8_t *r = get_vop (bp, ops);
  ...
  CASE (MIR_VGTI8, 3) {
    ...
    for (size_t i = 0; i < 16; i++)
      set_v128_u8 (r, i,
                   (int8_t) get_v128_u8 (op1, i) > (int8_t) get_v128_u8 (op2, i) ? UINT8_MAX
                                                                                  : 0);
    END_INSN;
  }
  CASE (MIR_VGTI16, 3) {
    ...
    for (size_t i = 0; i < 8; i++)
      set_v128_u16 (r, i,
                    (int16_t) get_v128_u16 (op1, i) > (int16_t) get_v128_u16 (op2, i)
                      ? UINT16_MAX
                      : 0);
    END_INSN;
  }
