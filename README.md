...
**Current status: 369 integration tests pass (0 failing). 261 unit tests pass (80 datadef + 24 IR + 5 libmadc_error + 133 libmadc_program + 19 libmadc_value). GCC torture test parity: at least 1503/1685 (89.2%) from the latest full sweep plus focused reruns. (`make -C src fulltest`, `scripts/run_gcc_testsuite.py`)**
...
| **GCC Parity** | GCC torture test suite compatibility | **v0.16.0+** — 1306/1685 (77.5%) passing; macro literal round-trip, unsigned promotion for comparisons, float-in-int-dest arithmetic, int→float cast natural type |
