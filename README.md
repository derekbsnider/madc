...
**Current status: 254 integration tests pass. 119 unit tests pass (77 datadef + 23 IR + 19 libmadc_value). (`make -C src fulltest`)**
...
| **Phase 4** | `libmadc.so` embedding API | **In progress** — §4.1 state split + structured diagnostics + engine-owned IO + full logging stack landed; §4.2 opened with `madc::value` at `include/libmadc/value.h` |
