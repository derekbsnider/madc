...
// Phase 5 sweep shapes (see interop_class.h): unary-before-binary operator
// pairs, a decaying array parameter, a class template instantiated here.
int &Counter::operator*() { return n; }
int Counter::operator*(int k) const { return n * k; }
Counter &Counter::operator++() { ++n; return *this; }
int Counter::operator++(int) { return n++; }
int Counter::add4(int vals[4])
{
    for (int i = 0; i < 4; ++i)
	n += vals[i];
    return n;
...
int via_fnptr_ref(int (*&fp)(int), int v) { return fp(v) + 1; }
int via_fn_ref(int (&fn)(int), int v) { return fn(v) + 2; }
