...
// One edit whose content is `nlines` lines "line0".."lineN-1" (no trailing
// newline), the caret parked at the start of `caret_line`, hinted rows =
// `rows`. Used to drive the viewport clip (virtualization): a large
// document must NOT emit every line every compose.
static uinode big_edit_tree(world &w, long nlines, long caret_line, long rows)
{
    roles r = roles::standard(w);
    uinode root(r.group);
    uinode edit(r.edit);
    ...
    {
	if ( i == caret_line )
	    caret = (long)text.size();
	text += "line" + std::to_string(i);
	if ( i + 1 < nlines )
	    text += "\n";
    }
    edit.content = madc::value(text);
    std::map<std::string, madc::value> h;
    h["caret"] = madc::value((int64_t)caret);
...
TEST_CASE("compose — a large edit clips to the viewport window (virtualization)")
{
    world w;
    roles r = roles::standard(w);
    web_model m;

    // 100 lines, caret at the top, a 10-row window: the compose must emit a
    // WINDOW of lines, not the whole document (the O(doc)-per-keystroke bug).
    nlohmann::json ops = nlohmann::json::parse(m.compose(r, big_edit_tree(w, 100, 0, 10)));
    const nlohmann::json *edit = node_by_key(ops, "0.0");
    REQUIRE(edit);
    CHECK((*edit)["class"] == "edit");
    REQUIRE((*edit).contains("total"));
    REQUIRE((*edit).contains("top"));
    CHECK((*edit)["total"] == 100);			// full geometry reported
    CHECK((*edit)["top"] == 0);				// caret at the top
    const nlohmann::json &lines = (*edit)["lines"];
    REQUIRE(lines.is_array());
    CHECK(lines.size() >= 10u);				// at least the visible rows
    CHECK(lines.size() < 100u);				// but NOT the whole document
    CHECK(lines.size() <= 42u);				// window + a bounded overscan
    CHECK(lines.front()["t"] == "line0");		// window starts at top
    // The caret line/col stay ABSOLUTE (the page maps them through `top`).
    CHECK((*edit)["caret"] == nlohmann::json{ {"line", 0}, {"col", 0} });

    // Caret near the end: the window scrolls to keep the caret visible, so
    // the emitted window is late in the document — still a window, not all.
    web_model m2;
    nlohmann::json ops2 = nlohmann::json::parse(m2.compose(r, big_edit_tree(w, 100, 90, 10)));
    const nlohmann::json *e2 = node_by_key(ops2, "0.0");
    REQUIRE(e2);
    CHECK((*e2)["total"] == 100);
    long top = (*e2)["top"].get<long>();
    const nlohmann::json &l2 = (*e2)["lines"];
    REQUIRE(l2.is_array());
    CHECK(l2.size() < 100u);
    CHECK(top > 0);					// scrolled down to the caret
    // The caret line (90) is inside the emitted window [top, top + size).
    long caret_line = (*e2)["caret"]["line"].get<long>();
    CHECK(caret_line == 90);
    CHECK(caret_line >= top);
    CHECK((size_t)(caret_line - top) < l2.size());
    // The window's first line text matches its absolute index.
    CHECK(l2.front()["t"] == "line" + std::to_string(top));
}

TEST_CASE("apply_input — a scroll message moves the window without snapping to the caret")
{
    world w;
    roles r = roles::standard(w);
    web_model m;
    // 100 lines, caret at line 0, a 10-row window: the initial window is at
    // the top (the single edit's node key is "0.0").
    m.compose(r, big_edit_tree(w, 100, 0, 10));
    // The page scrolls to line 50 (mouse-wheel / scrollbar).
    std::vector<tui_event> ev = m.apply_input("{\"kind\":\"scroll\",\"key\":\"0.0\",\"top\":50}");
    REQUIRE(ev.size() == 1u);
    CHECK(ev[0].kind == tui_event_kind::scroll);
    // Recompose with the SAME caret (unchanged): the window follows the
    // scroll, not the caret — it stays around line 50, not snapped to 0.
    nlohmann::json ops = nlohmann::json::parse(m.compose(r, big_edit_tree(w, 100, 0, 10)));
    const nlohmann::json *edit = node_by_key(ops, "0.0");
    REQUIRE(edit);
    long top = (*edit)["top"].get<long>();
    CHECK(top > 30);					// near line 50, not 0
    CHECK((*edit)["lines"].front()["t"] == "line" + std::to_string(top));
    // A malformed scroll (missing top / missing key / negative) yields none.
    CHECK(m.apply_input("{\"kind\":\"scroll\",\"key\":\"0.0\"}").empty());
    CHECK(m.apply_input("{\"kind\":\"scroll\",\"top\":5}").empty());
    CHECK(m.apply_input("{\"kind\":\"scroll\",\"key\":\"0.0\",\"top\":-1}").empty());
}

TEST_CASE("compose — a small edit still emits every line (clip is a no-op under the window)")
{
    world w;
    roles r = roles::standard(w);
    web_model m;
    // 3 lines, a 40-row window: the whole document fits, so top=0 and every
    // line is emitted — the small-file path is unchanged but reports geometry.
    nlohmann::json ops = nlohmann::json::parse(m.compose(r, big_edit_tree(w, 3, 0, 40)));
    const nlohmann::json *edit = node_by_key(ops, "0.0");
    REQUIRE(edit);
    CHECK((*edit)["top"] == 0);
    CHECK((*edit)["total"] == 3);
    ...
    CHECK(lines[0]["t"] == "line0");
    CHECK(lines[2]["t"] == "line2");
}
