...
TEST_SUITE("libmadc sqlite backend") {

#ifdef HAVE_SQLITE3
    TEST_CASE("sqlite dataset stores keyed records across reopen") {
	const std::string path =
	    "/tmp/madc_sqlite_probe_" + std::to_string(static_cast<long long>(getpid())) + ".db";
	...
	{
	    madc::DataSet<StorageProbe> ds("sqlite://" + path);
...

	{
	    ...
	    CHECK(short_name_string(fetched) == "CHARLIE");
	    ...
	    StorageProbe bob = make_probe(2, 99, 9, true, StorageStatus::active, 'B',
					  "BRAVO", "Bob Updated", 9.75);
		...
		ds.query(madc::query().from("users").where_ne("short_name", madc::value("ALPHA")).limit(2).build(), &err);
	    REQUIRE(static_cast<bool>(not_equal));
	    REQUIRE(not_equal->next(row));
	    CHECK(row.id == 3);
	    CHECK(row.short_name == std::string("CHARLIE"));
		...
		ds.query_raw(madc::query().from("users").where_eq("enabled", madc::value(true)).select(std::vector<std::string>{"id", "short_name"}).limit(1).build(), &err);
	    ...
	    CHECK(projected_row.as_object().count("short_name") == 1);
	    CHECK(projected_row.as_object().count("title") == 0);
	    CHECK(projected_row.as_object().at("id") == madc::value(int64_t(1)));
	    CHECK(projected_row.as_object().at("short_name") == madc::value("ALPHA"));
...
#else
    TEST_CASE("sqlite backend is not built in this configuration") {
	CHECK(true);
    }
#endif
}
