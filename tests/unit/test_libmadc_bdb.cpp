...
TEST_SUITE("libmadc bdb backend") {

#ifdef HAVE_BDB
    TEST_CASE("bdb dataset stores keyed records in Btree key order") {
	const std::string path =
	    "/tmp/madc_bdb_probe_" + std::to_string(static_cast<long long>(getpid())) + ".db";
...

	{
	    ...
	    madc::DataSet<StorageProbe> ds("bdb://" + path);
...
#else
    TEST_CASE("bdb backend is not built in this configuration") {
	CHECK(true);
    }
#endif
}
