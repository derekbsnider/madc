...
public:
	...
	// MESSAGE mode — the framing the Language Server Protocol (and DAP, and
	// BSP) uses: a header block, a blank line, then exactly that many bytes.
	// A mode switch, not a handshake. Afterwards readline() returns one
	// complete message body and write() emits the header plus the body;
	// read()/readall() refuse (message-oriented), as they do for a
	// WebSocket. false (with last_error) on a channel with no endpoint, or
	// one already speaking another framing — a channel speaks one framing.
	bool frame_headers();
