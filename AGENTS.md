...
| [indirection.md](.claude/rules/indirection.md) |    44 | **ONE owner per layered-pointer/reference concern**, indexed: the `*` operand is `parseCastExpression` (the engine, bounded) + `build_indirection` (gated by `check-one-deref-builder.sh`); type minting/peeling, an array operand's element (`array_operand_element_type`), decay, declarators, symbol counting (angle brackets → `delimiter-tracking.md`) |

### Total rule footprint

- **36 rules, 1137 lines** in `.claude/rules/` (per `scripts/rule_stats.sh`).
