...
// These extern "C" wrappers operate on void* object pointers and are      //
// resolved via dlsym(RTLD_DEFAULT) at MIR-link time for the              //
// CIR -> c2mir -> MIR backend. The emitted C declares each symbol as an   //
// extern prototype (see CirBuilder::need_output_extern); MIR links it to  //
// the matching wrapper below.                                             //
//                                                                         //
///////////////////////////////////////////////////////////////////////////
...
// Push a std::string OBJECT (madc string buffer) by value.
void vector_str_push_back(void *ptr, void *str)
{
    ((std::vector<std::string> *)ptr)->push_back(*(std::string *)str);
}

// Push from a const char* (string literal / char* value).
void vector_str_push_back_cstr(void *ptr, const char *str)
{
    ((std::vector<std::string> *)ptr)->push_back(str ? std::string(str) : std::string());
}
...
// Fill `result` (a std::string object) with element `index`; returns result.
...
void vector_str_set_cstr(void *ptr, int64_t index, const char *str)
{
    std::vector<std::string> &v = *(std::vector<std::string> *)ptr;
    if ( index >= 0 && (size_t)index < v.size() )
        v[(size_t)index] = str ? std::string(str) : std::string();
}
...
void map_str_int_set_cstr(void *ptr, const char *key, int64_t val)
{
    (*(std::map<std::string, int64_t> *)ptr)[key ? std::string(key) : std::string()] = val;
}
...
int64_t map_str_int_get_cstr(void *ptr, const char *key)
{
    std::map<std::string, int64_t> &m = *(std::map<std::string, int64_t> *)ptr;
    auto it = m.find(key ? std::string(key) : std::string());
    return it != m.end() ? it->second : 0;
}
...
void map_str_str_set_cstr(void *ptr, const char *key, const char *val)
{
    (*(std::map<std::string, std::string> *)ptr)[key ? std::string(key) : std::string()]
        = val ? std::string(val) : std::string();
}

// Fill `result` (a std::string object) with the value for `key`; returns result.
void *map_str_str_get(void *result, void *ptr, void *key)
{
    std::map<std::string, std::string> &m = *(std::map<std::string, std::string> *)ptr;
    std::string &res = *(std::string *)result;
    auto it = m.find(*(std::string *)key);
    res = it != m.end() ? it->second : std::string();
    return result;
}
...
void map_str_str_clear(void *ptr)
{
    ((std::map<std::string, std::string> *)ptr)->clear();
}
...
void set_str_insert_cstr(void *ptr, const char *str)
{
    ((std::set<std::string> *)ptr)->insert(str ? std::string(str) : std::string());
}
