...
if [ "$_cur_branch" = "feature/header-partition-claude" ]; then
  ...
  echo "  HEAD f77ffa8; fulltest 543/4; gcc.c-torture 1566/31/57/1. 28 commits ahead of develop, LOCAL/UNPUSHED."
  echo "  LIVE FRONT = real <string> (gates getline -> testfstream/testloop). Root-caused to the"
  echo "  __void_t SFINAE DETECTION IDIOM for iterator_traits (handoff §12 has the 4-piece plan +"
  echo "  g++ oracle: iterator_traits<__normal_iterator<char*,string>>::iterator_category ="
  echo "  std::random_access_iterator_tag). PREREQUISITE STASHED (stash@{0}: nested template-id"
  echo "  partial-spec unifier for allocator_traits<allocator<_Tp>> — CORRECT but regresses cout"
  echo "  ALONE until the detection idiom lands; do NOT commit alone). HEAD is GREEN with it stashed."
  echo "  User steering 2026-06-09: build the REAL detection idiom (Rule #2, NO shims, don't even"
  echo "  OFFER them); lazy member-typedef instantiation is a follow-up, not a substitute; g++ oracle."
