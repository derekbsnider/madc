...
if [ "$_cur_branch" = "feature/header-partition-claude" ]; then
  ...
  echo "  W2 DONE: step-1 capture acd9fdc, step-2 bind 61d9264 (cout<<\"lit\" binds the FREE"
  echo "  std::operator<< -> exact g++ symbol _ZStlsISt11char_traitsIcEERSt13basic_ostreamIcT_ES5_PKc"
  echo "  via try_free_operator_call in cir_builder.cpp). External-dtor synthesis gated e9a2792"
  echo "  (libstdc++ owns std exception dtors; killed the dangling _ZNSt12__cow_stringD1Ev)."
  echo "  real <iostream> went 2858 c2mir errors -> 0; now ONE MIR-link gap remains:"
  echo "  NEXT = bind std::endl mangled-direct. 'cout << endl' emits a CALL to an UNDEFINED"
  echo "  __ns_std_endl() (namespace_function_symbol, parser.cpp:~1504) instead of passing"
  echo "  std::endl as the manipulator FUNCTION POINTER bound to the real"
  echo "  _ZSt4endlIcSt11char_traitsIcEERSt13basic_ostreamIT_T0_ES6_ (W1 mangler already has the"
  echo "  endl test). Free-function-VALUE binding (reuse W2 free-template deduction +"
  echo "  itanium_mangle_std_free_template), distinct from the operator-call path. Then real"
  echo "  <iostream> RUNS end-to-end (add a .flags/.expect test). Broader W2: operator<< for"
  echo "  std::string, operator>>, getline; then friend operators + full ADL (task #19)."
  echo "  fulltest 542/4; gcc.c-torture 1566/31/57/1. 16 commits ahead of develop, local."
