...
if [ "$_cur_branch" = "feature/header-partition-claude" ]; then
  echo "ACTIVE LINE (this branch): READ FIRST -> docs/plans/2026-06-08-header-partition-W2-perf-HANDOFF.md"
  ...
  echo "  MILESTONES this session: (1) real libstdc++ <iostream> COMPILES+RUNS end-to-end"
  echo "  (testcout.mad --std=c++17 --no-embedded-headers -> 'This is a test, x = -1', = g++),"
  echo "  via W2 non-member operator resolution + std::endl manipulator + facet vtable ownership"
  echo "  + reachability DCE + external-dtor gating (commits 32484e1,12398f2,acd9fdc,61d9264,"
  echo "  e9a2792,f38c005). (2) parser O(n^2) FIXED: TokenCpnd::findVariable linear scan -> O(1)"
  echo "  index (10fcef1) -> real <iostream> compile 7.6s -> 1.33s (g++ 0.48s)."
  echo "  HEAD 10fcef1; fulltest 543/4; gcc.c-torture 1566/31/57/1. 17 commits ahead of develop, local."
  echo "  NEXT (pick a lever, all in the handoff §6/§7): perf eager-instantiation reduction"
  echo "  ('no instantiation inside system headers' - user-flagged, biggest, delicate; B3-B6 PCH"
  echo "  solves it wholesale) | lexer buffering | unordered_map the other symbol tables |"
  echo "  W2 breadth (std::string operator<</operator>>/getline) | friend+full-ADL (#19) |"
  echo "  R1 per-std macros+__has_* | R3 retire array keyword | A acceptance oracle | M shim retirement."
  echo "  (Supersedes 2026-06-08-system-header-dce-HANDOFF.md.)"
