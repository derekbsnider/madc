...
if [ "$_cur_branch" = "feature/header-partition-claude" ]; then
  ...
  echo "  top ⚑ turn-3 banner, THEN docs/plans/2026-06-09-lazy-member-body-instantiation-plan.md §8."
  echo "  HEAD ~2173ae0 = green tip + PROVEN __void_t detection idiom + LAZY member-function-body"
  echo "  instantiation ([temp.inst]: defer system-header member bodies, materialize on ODR-use)."
  echo "  Build clean. EFFECT: testcout (real <iostream>) now ADVANCES PAST the _M_local_data"
  echo "  'Unknown namespace pointer' wall; str1 still hits it (std::string's dtor genuinely odr-uses"
  echo "  _M_local_data). OFF the green tip on purpose (mid-chain reds)."
  echo "  TWO FRONTIERS (plan §8): (1) testcout: allocator_int32_t___dtor UNDEFINED (call-vs-def"
  echo "  mismatch / late-instantiation synth-dtor gap; int is REAL = char_traits<char>::int_type)."
  echo "  (2) str1 HEADLINE: implement 'extern template' (madc has NONE) + bind extern-template"
  echo "  instantiations' members to libstdc++ C1/D1/mangled (basic_string<char> is non-poly so"
  echo "  is_externally_defined bails at !has_vtable, parser.cpp:6316). Do NOT blanket-bind all"
  echo "  system non-poly instantiations (vector<int> not exported). Reducer tmp/str1.mad; g++ oracle."
  echo "  Rule #2 NO shims (don't even OFFER them); gate fulltest+torture-ALONE+canaries before the"
  echo "  chain lands on green. DO NOT push without asking."
