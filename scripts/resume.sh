...
if [ "$_cur_branch" = "feature/header-partition-claude" ]; then
  ...
  echo "  correctly; the WALL is a DESTRUCTOR SIGSEGV at scope exit (root UNCONFIRMED — see §5;"
  echo "  the --emit=c11 'array-of-object member renders scalar' signal may be a text artifact,"
  echo "  member_node has the right dims). Keystone landed: mangled-direct construction/destruction"
  echo "  for externally-defined classes (6b5d4ea) + combined-typedef hoist + external complete-dtor"
  echo "  (38d9152) + W2 derived-stream operator binding (22c5b53) + instantiation-cache/is_complete"
  echo "  (39878b7) + by-value struct ordering (e6beebc) + vptr-in-typedef (e4fca20)."
  echo "  HEAD 22c5b53; fulltest 543/4; gcc.c-torture 1566/31/57/1. 24 commits ahead of develop, LOCAL/UNPUSHED."
  echo "  NEXT: diagnose the dtor crash via RUNTIME (gdb the JIT SIGSEGV / real sizeof vs g++ 512 /"
  echo "  vbase-offset +248 vs g++), NOT --emit=c11 text (handoff §5). Then ifstream/getline cluster,"
