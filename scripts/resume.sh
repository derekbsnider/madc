...
if [ "$_cur_branch" = "feature/header-partition-claude" ]; then
  echo "ACTIVE LINE (this branch): READ FIRST -> docs/plans/2026-06-08-system-header-dce-HANDOFF.md"
  echo "  HEADER-PARTITION campaign: madc consumes REAL libstdc++ (retire hand-tooled shims)."
  echo "  HEAD 86d868d; fulltest 542/4; gcc.c-torture 1566/31/57/1. Plan: ~/.claude/plans/clever-scribbling-dove.md"
  echo "  DONE this session: typedef-collision (b9d667f), diamond-with-data vbase layout (e9dbef1),"
  echo "  VTABLE OWNERSHIP runtime-proven (addfee3 predicate+suppress, 86d868d real _ZTVSt/_ZTISt)."
  echo "  NEXT = (a) system-header-origin + reachability DCE for dead library inline-method bodies"
  echo "  (real <iostream> at 21 errors, all dead) + Tier 2 extern-template mangled-direct. See handoff."
  echo "  (Prior 2026-06-08-header-partition-HANDOFF.md is superseded for current state.)"
