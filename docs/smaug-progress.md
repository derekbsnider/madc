...
| **Runtime**      | ~40%+ | With proper data directories (run from `area/` with `../gods/`, `../system/`, `../races/`, `../classes/`) and the post-session-11 fixes, boot_db reaches: commands → sysdata → socials → skill table → classes → races → herbs → tongues → wizlist (shim) → retiredlist (shim) → request pipe → random number init → time/weather → `Assigning gsn's` (next blocker, inside skill_lookup). Three madc fixes landed during the session: TokenMember::operand LEA for fixed-array members (so `dentry->d_name` decays to char* not first-byte char), TokenCpnd::voperand re-emit for global fixed-arrays (so the base pointer is loaded on every branch), TokenAddrExpr branch for &arr[i] (so address-of subscript LEAs instead of loading the value). |
