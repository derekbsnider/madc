...
| **Runtime**      | ~75%+ | With proper data directories, **all 25 area files load end-to-end** AND area_update fires (Resetting: newacad.are, newgate.are, ...) AND board loading begins. Ten madc fixes this session: TokenMember LEA for fixed-array members, TokenCpnd voperand re-emit for global fixed-arrays, TokenAddrExpr for &arr[i], static-local struct persistence, parseDeclaration extern→def storage, TokenCallFunc small-struct (rax,rdx) return-spill, TokenSubscriptExpr operand() lvalue Mem, lexer file-pointer interning, JIT-crash → source-line tooling, and **TokenLand/TokenLor actually short-circuit `&&`/`||`** (which loaded right operand even when left was NULL — the canonical `if (p && p->next)` segfaulted on NULL p). Most of these were uncovered in <5 minutes apiece once the source-line tooling was in. (rest of historical entry preserved below) — gods.are, limbo.are, newacad/newgate/newdark, haon, midennir, sewer, redferne, grove, dwarven, daycare, grave, chapel, astral, Build, pixie, export, srefuge, manor, unholy, gallery, plus 3 help files. Crashes inside `init_supermob` (after the area loop, in `create_mobile` / `char_to_room` field setup — likely another struct-copy or pointer-as-value pattern). Seven madc fixes this session: TokenMember LEA for fixed-array members, TokenCpnd::voperand re-emit for global fixed-arrays, TokenAddrExpr branch for &arr[i], static-local struct persistence, parseDeclaration extern→def storage allocation, TokenCallFunc small-struct (rax, rdx) return-spill, TokenSubscriptExpr operand() override (lvalue Mem instead of value-load — fixed 2D struct-member array writes). |
