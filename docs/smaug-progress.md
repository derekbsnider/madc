...
| **Runtime**      | ~55%+ | With proper data directories and the post-session-11 madc fixes, boot_db loads `gods.are` end-to-end with the canonical SMAUG summary: `gods.are : Rooms: 1200-1201 Objs: 1200-1200 Mobs: 1200-1200`. Crashes (NULL deref) loading the next area-list entry (limbo.are). Six madc fixes landed: TokenMember::operand LEA for fixed-array members, TokenCpnd::voperand re-emit for global fixed-arrays, TokenAddrExpr branch for &arr[i], static-local struct persistence (TokenSTATIC threaded into parseDeclaration via Program::parsing_static_decl), parseDeclaration extern→def storage allocation, and TokenCallFunc spill (rax, rdx) to stack for small-struct return-by-value (SysV ABI gap — fread_bitvector returns EXT_BV in 16-byte register pair). |
