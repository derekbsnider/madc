...
| **Runtime**      | ~50%+ | With proper data directories and the post-session-11 fixes, boot_db reaches: commands → sysdata → socials → skill table → classes → races → herbs → tongues → wizlist (shim) → retiredlist (shim) → request pipe → random number init → time/weather → gsn's (skill_lookup stubbed) → area-list loading: help.are / imc-help.are / ice-help.are all parse end-to-end (with expected duplicate-help warnings), then crashes loading gods.are when the mobile section's act_flags integer (1073741825 = 0x40000001) is treated as a pointer. Five madc fixes landed: TokenMember::operand LEA for fixed-array members, TokenCpnd::voperand re-emit for global fixed-arrays, TokenAddrExpr branch for &arr[i], static-local struct persistence (TokenSTATIC threaded into parseDeclaration via Program::parsing_static_decl), and parseDeclaration extern→def storage allocation (so cross-TU globals like last_area unify across functions instead of becoming per-function stack copies). |
