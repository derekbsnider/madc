# GCC Methodology — Reasoning

## Why GCC is canon

madc's goal is C compatibility. GCC is the reference C compiler. When madc
and GCC disagree on a test's behavior, madc is wrong — not GCC. This is true
even when madc's approach seems "simpler" or "more efficient."

`gcc -S -fverbose-asm -O0` is the gold standard because:
...
- The output shows exactly what registers are used, what width they are,
  and what instruction (signed vs unsigned) was chosen.
