...
Test results as of May 6, 2026 (post-v0.13.0 plus first-wave C23 compatibility and ongoing Phase 4.2/4.3 libmadc API work: `madc::value`, `madc::error`, the first `madc::program` pimpl slice with narrow callback registration, script-call, global get/set, first entry-function-based `eval(...)`, a first thin `madc::eval(...)` / `madc::exec_*` convenience wrapper tier, explicit host-side `eval_unit(...)` aliases for the full-translation-unit lane, typed host-side `madc::program::eval(...)` / `eval_expression(...)` overloads and matching top-level typed `madc::eval(...)` / `eval_expression(...)` wrappers for `bool` / `int64_t` / `double` / `std::string`, a new explicit host-side `eval_body(...)` lane with typed overloads plus `program::native_type` for generic body-text runtime eval, typed `register_function(name, fn)` callback deduction for supported plain C++ function pointers including `std::string`-backed callback parameters and returns, a first `lib/libmadc.so` target with dedicated PIC objects and `install-libmadc`, a thin C ABI in `include/madc_api.h` / `src/madc_c_api.cpp` for opaque program handles and scalar/string `madc_value` exchange, first `eval_expression(...)` plus a dedicated `expression_policy` surface for header-group/function allowlists with `math.h` as the first real header group, a dedicated `runtime_eval_policy` surface for full script-side child eval programs, an AST-level whitelist on the dedicated expression seam, explicit gates for function calls, member access, subscript access, and pointer operations, a first host-supplied binding seam for scalar/string expression variables, an object-backed expression context seam that now resolves top-level and nested static primitive object paths through a parser-owned named-root/context-object model while keeping pointer semantics out of bounds, a first in-language `madc::eval_expression(out, expr)` bridge plus typed runtime helpers `madc::eval_expression_int(expr)`, `madc::eval_expression_bool(expr)`, `madc::eval_expression_double(expr)`, and exact-string `madc::eval_expression_string(out, expr)` for script code, plus typed full-source `madc::eval_bool/int/double/string(...)` helpers that now auto-wrap body text into `__madc_eval` when no explicit entry function is provided, plus `MadArray`-backed runtime context helpers `madc::context_set_int/real/string/array(...)` and `madc::eval_expression_*_ctx(...)`, plus full in-language `madc::eval(out, source)` with typed `eval_int/bool/double` and exact-string `eval_string(...)` helpers on the entry-function-based runtime eval seam, plus independently gated current-scope capture for script-side `madc::eval*` and `madc::eval_expression*` through a compiler-synthesized `MadArray` context object with separate source-vs-expression scope-access gates, with lexer-token call gating before parse, first options/policy surfaces, first-pass policy gating for `#load` plus raw `dlsym` fallback, enforced `authority_mode::system_locked` clamping for process/dlfcn capability and both runtime-eval scope-access gates, an explicit `execution_mode` seam with real fork-per-invocation child execution for `exec_file(...)` / `exec_string(...)` plus the current narrow scalar/C-string `call(...)` and `eval(...)` path, and post-invocation invoke-limit enforcement for CPU time, resident growth, and both managed-stream plus raw-libc output bytes, logging lifecycle fixes, exploratory storage/federation API contracts, working `dsv://` + `flr://` + `vlr://` + `qdbm://` + `gdbm://` + `bdb://` + `sqlite://` backend slices, FLR tombstone sidecars with pre-reap restore plus post-reap restore-by-archive reinsertion, the first concrete FLR->VLR offset relation slice, stable append-only VLR locators behind tombstone sidecars, first-wave query pushdown through `DataSet<T>::query(...)`, bounded plus strict key-range pushdown for SQLite and ordered keyed stores, first relation-aware traversal across filtered source rows, raw projected builder queries through `DataSet<T>::query_raw(...)`, builder `where_ne(...)`, `where_in(...)`, `where_not_in(...)`, and `where_like(...)` support with SQLite pushdown plus keyed-store fallback, and a first logical-composition seam through query match-mode metadata (`all` vs `any`) with explicit runtime rejection for non-default composition until OR semantics are implemented, plus `DataSource` domain/family classification as a core storage/service/IPC conduit type, and registration-based `infer_mapper()` for host C++ storage types).
...
## Current Batch Status — 259 passed, 0 failed, 0 timed out, 0 skipped
...
- Failing: none
- Timed out: none
- Unit tests: all passing (doctest, core-only `--enable-madcdat=no` workspace) — 80 datadef + 23 IR + 97 libmadc_program + 5 libmadc_error + 19 libmadc_value (224 total)
