...
Test results as of May 3, 2026 (post-v0.13.0 plus first-wave C23 compatibility and ongoing Phase 4.2 libmadc API work: `madc::value`, `madc::error`, logging lifecycle fixes, exploratory storage/federation API contracts, working `dsv://` + `flr://` + `vlr://` + `qdbm://` + `gdbm://` + `bdb://` + `sqlite://` backend slices, FLR tombstone sidecars with pre-reap restore plus post-reap restore-by-archive reinsertion, the first concrete FLR->VLR offset relation slice, stable append-only VLR locators behind tombstone sidecars, first-wave query pushdown through `DataSet<T>::query(...)`, bounded key-range pushdown for SQLite and ordered keyed stores, first relation-aware traversal across filtered source rows, raw projected builder queries through `DataSet<T>::query_raw(...)`, `DataSource` domain/family classification as a core storage/service/IPC conduit type, and registration-based `infer_mapper()` for host C++ storage types).
...
- Failing: none
- Timed out: none
- Unit tests: all passing (doctest) — 80 datadef + 23 IR + 1 libmadc_bdb + 2 libmadc_dsv + 5 libmadc_flr + 1 libmadc_gdbm + 1 libmadc_qdbm + 2 libmadc_relation + 1 libmadc_sqlite + 5 libmadc_error + 19 libmadc_value + 3 libmadc_vlr + 19 libmadc_storage_contract
