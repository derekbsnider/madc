...
| cluster | tests | notes |
|---|---|---|
| **`__int128`** | pr98474, pr122943, pr63302 (3) | Widely used (crypto/hash). madc already predefines `__SIZEOF_INT128__` without providing the type — the tests self-select the 128-bit branch and lose. Verified: the long-long halves pass in isolation. Already a value-pool roadmap concern. Interim option: stop predefining the macro until the type lands (tests then self-select their LL path). |
...
| **packed + misaligned access** | misalign, 20020227-1 (2) | GNU `packed` is ubiquitous (protocol structs). misalign = layout/init bug on packed stores; 20020227-1 = `_Complex float` member in a packed struct → MIR internal error. |
