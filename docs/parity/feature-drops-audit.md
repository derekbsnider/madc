...
| 7 | inline `static_cast<Derived*>(base)->member` (drives `_M_valptr` on map/set iterators) | REJECT (parse) — ROOT-CAUSED; fix STASHED w/ a residual leak | `map`/`set` element access via `operator[]`/iterator-deref fails `error: no member named '_M_valptr'`. ROOT (gdb + minimal reducer `tmp/sc2.cpp` `static_cast<Derived*>(bp)->get()`): an INLINE named-cast followed by postfix `->m()` has the postfix folded INTO the cast OPERAND, so it resolves against the operand's (base) type, not the cast's target. Cause: `parseExpr_operatorArm` (parser.cpp ~17449) deliberately continues a parenthesized SUB-expression into a trailing `->`/`.`/`[]` (so `&((ch)->next)` works) — but `parse_named_cpp_cast` reuses that same operand parse, so `static_cast<T>(p)->m()` over-consumes `p)->m()` as the operand. libstdc++'s `_Rb_tree_iterator::operator*` is exactly `*static_cast<_Link_type>(_M_node)->_M_valptr()`. FIX (stashed `git stash` "WIP _M_valptr fix"): parse the cast operand in an ISOLATED token stream (balanced `( )`) so it stops at its own `)`, + route the cast result through `parsePostfixChainFrom`. Fixes sc1/sc2 and advances map past `_M_valptr` (to a further "Missing operand" wall). BLOCKER: an order-dependent unit-test leak — `test_libmadc_program` 131/1 (a `*string*` case that passes in isolation). Restored tokens/_cur_token/_prv_token/diagnostics/last_error/static-parse-pos — leak persists; some OTHER global/static is left mutated by the isolated parse. NEXT: find the leaked global (compounds? class_scope_stack? a parser flag?), OR take the alternative — gate the ~17449 postfix-continuation off for a cast operand (a threaded flag) instead of isolating. NB the "1 failed" was ONLY visible after the Makefile stale-test-binary fix (3491ee2) — earlier runs tested a stale binary. | 2026-06-16 | `tmp/sc2.cpp` (minimal); `tmp/tp2.cpp`, `tmp/mvi.cpp` (map/set) |
