...
| **V3 Clients + windows** (V3a ✅ anchor registry · V3b ✅ event_any + multi-client loop + viewwindow · V3c ◑ presence: V3c-1 ✅ shift+draw machinery, V3c-2 ⏳ palette+render) | client records; `ui::event_any`; `viewwindow` opens a second window on the session; presence carets + `@presence` colours; the anchor registry replaces `shift_hspans` (V3c-1: the doc carries a roster of viewing es — `es_view_doc` — dealt a round-robin colour slot, `shift_anchors` shifts EVERY client's caret in one pass not just the editing es, `compose_edit_node` draws the others as carets in their slot; V3c-2: the `@presence` palette slot→colour + the web render) | `tests/testmadcide_window2` (two clients, one edit seen in both; per-client carets; viewwindow park vs refuse — fake-host/direct-drive, no display); `testuieventany`; `check-one-anchor-owner.sh`. The real two-window webview pump is the flagged GTK-smoke + seam-lane follow-up |
