...
| **V3 Clients + windows** (V3a ✅ anchor registry · V3b ✅ event_any + multi-client loop + viewwindow · V3c ⏳ presence) | client records; `ui::event_any`; `viewwindow` opens a second window on the session; presence carets + `@presence` colours; the anchor registry replaces `shift_hspans` | `tests/testmadcide_window2` (two clients, one edit seen in both; per-client carets; viewwindow park vs refuse — fake-host/direct-drive, no display); `testuieventany`; `check-one-anchor-owner.sh`. The real two-window webview pump is the flagged GTK-smoke + seam-lane follow-up |
