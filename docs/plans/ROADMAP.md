...
| 2.11 | **Self-host: madc compiles its own C++11 source** (ultimate dogfood) | large | **IN PROGRESS (started 2026-09-17, owner go)** — the harness is LIVE: `scripts/selfhost_lane.sh` / `make -C src selfhost` runs every TU the Makefile links (72, flags derived by `make -n`) + 100 own-header wrapper TUs through `--emit=c11`, ratcheted over `docs/parity/selfhost-baseline.txt`; **49 of 172 units pass** (7 rt/*.c, 6 src TUs, 36 headers) after the first four fixes: `thread_local`/`_Thread_local` (+ a c2mir block-scope fix), unnamed namespaces, `dynamic_cast<const T*>`, `typedef enum : T {}`; next by yield: class `operator[]` element access on reference/member receivers (43 units), pointer-to-member-function type (29), `tuple_element<__i, const _Tp>` partial specialization (20), the `<regex>` recovery loop, tsubst deferred-construction packs (10) | failure-driven `selfhost` harness |
