...
| 2.11 | **Self-host: madc compiles its own C++11 source** (ultimate dogfood) | large | **STARTED 2026-09-17 (owner go)** — harness first: a failure-driven `selfhost` lane over every src/include TU with the real build flags, baseline-ratcheted; measured 2026-09-17: lexer.cpp 338 errors / 57 shapes, first blocker `extern thread_local` (datadef.h:23), then pointer-to-member declarators in `<functional>`; the parse is what the Nexus needs to serve madc's own development | failure-driven `selfhost` harness |
