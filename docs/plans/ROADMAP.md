...
| 1.8 | **`import` — the module binding** (owner 2026-09-06: the C++20 module import made whole — interface AND binding; `#load` retires, `-l` becomes the build-system spelling of the same resolver). Module map as data (name → interface + neutral library name), ONE platform-spelling owner (fixes the Windows-less `MADC_DSO_SUFFIX` `-l` mapping), per-TARGET binding policy (JIT open · link-time import · startup-time open emitted into the artifact — closes the `testdlopen.exe_skip` follow-on), keyword gated by `--std=` through the registry | 2-3 wk | **Designed 2026-09-06** — prerequisite of 7.5 | [design §3.1](2026-09-06-ui-web-target-and-madcide-gui.md) |
