...
| 2.10 | **Single-name local instantiations (flattened→Itanium-mangled)** | 1-2 wk | **Partial (2026-09-17)** — the C++ symbol mangling feature (merge `f266d38d8`) names every USER-defined C++ symbol Itanium, including user function-/member-template products; LIBRARY-pattern (libstdc++/libc++) instantiation bodies still emit under flattened keys — the ODR-merge leg is open (default: stay internal until decided) | [2026-09-16-free-function-overloading-linkage.md](2026-09-16-free-function-overloading-linkage.md) |
| 2.11 | **Self-host: madc compiles its own C++11 source** (ultimate dogfood) | large | **IN PROGRESS (started 2026-09-17, owner go)** — the harness is LIVE: `scripts/selfhost_lane.sh` / `make -C src selfhost` runs every TU the Makefile links (72, flags derived by `make -n`) + 100 own-header wrapper TUs through `--emit=c11`, ratcheted over `docs/parity/selfhost-baseline.txt`; **69 of 172 units pass** (7 rt/*.c, 13 src TUs, 49 headers) after: `thread_local`/`_Thread_local` (+ a c2mir block-scope fix), unnamed namespaces, `dynamic_cast<const T*>`, `typedef enum : T {}`, class `operator[]` element typing, qualified class-scoped constants, `auto *p = new T`, global-qualified names, pointer-to-member functions and data members (types + values), namespace-function self-recursion, `using ns::alias_template;`, method-call Derived*->Base* argument upcast, cast-operand heads through the entry resolver, a LITERAL-only ternary prune (const& member reads are not constants), a class nested in a data-only aggregate, a deduced type pack in a fn-template return (`std::get<I>`) + the unifier honouring a bound non-type param over the base chain, struct members `*const *`, the class-name declared at its class-head (CRTP bases) + [class.mem]/6 complete-class body deferral; next by yield: tsubst deferred-construction object pack (42; root found), nested aggregate tags in data-only aggregates (17), `vector<bool>` member-definition selection (13), function types as parameters/template arguments (8) | failure-driven `selfhost` harness |
| 2.12 | **C++11 conformance measured by a third-party suite** — gcc's g++ testsuite (`g++.dg` cpp0x+template compile-clean, `--std=c++11`) driven by the gcc-torture runner, ratcheted against a baseline | first measurement DONE | **1464/1950 in-scope (75.1%) at 2026-09-20 — 🎯 THE OWNER'S 75% TRIGGER IS REACHED**, baseline 486, 0 outside baseline. EVERY measured era now published in [docs/conformance-coverage.md](../conformance-coverage.md): C++98 308/385 (80.0%), C++11 1151/1565 (73.5%) for cpp0x alone, C++14 206/417 (49.4%), C++17 128/308 (41.6%), C++20 346/653 (53.0%); C torture 1611/1624 (99.2%) under c17 and c-testsuite 220/220. NEXT PUSH IS C++98 (owner 2026-09-20): g++.dg/template is shared by every era, so its 77 remaining failures drag on all of the rows above at once, 0 runtime failures (was 1461/74.9% after T3, 1456/74.7% after T1b, 1454/74.6% after SFINAE T1, 1441/73.9% at the declarator-arc close, 1440/73.8% after T10, 1430/73.3% after T6, 1412/72.4% on 2026-09-18, 1169/60% on 2026-09-17). The 2026-09-19 gain (+29) is the declarator-reader consolidation T1–T13 COMPLETE ([2026-09-19-declarator-reader-consolidation.md](2026-09-19-declarator-reader-consolidation.md)): ONE `Program::parse_declarator` adopted by every arm — alias, template-argument, cast/sizeof, typedef, member, K&R and C++ parameter, template-parameter and variable declarators; fourteen private readers → ONE, gated (`scripts/check-one-declarator-reader.sh` ratcheted 15/17/10/6/3 → 2/5/6/3/2, two-way self-test); the standing instance is in AGENTS.md Rule #4. **2026-09-20: expression SFINAE T1 (de3e78261, [2026-09-19-expression-sfinae.md](2026-09-19-expression-sfinae.md)) +13** — a substitution failure while forming a function template's declared type is SILENT and declines the candidate (the trap caught the throw but the rendered+recorded diagnostic refused the unit), and a defaulted type parameter no function parameter names has its default substituted in the explicit-args lane too (failure = not viable). **T1b (c164fd10b) +3**: the sfinae fallback idiom `char (&f(...))[2]` registers (a parenthesized declarator-id) and resolves through the ONE declarator reader; a call on a void/arithmetic call result is refused ([expr.call]/1). sfinae-nullptr1 moved INTO the baseline with its true cause (no distinct `nullptr_t`; KG Gap nullptr_t_distinct_type). **T3 (8eab5bc4c) +5**: a construction madc can PROVE ill-formed is refused at the parse — reference value-init, function type, abstract class (a parser-side final-overrider walk; the method map is pre-filled with base entries), deleted default ctor, deleted dtor of a temporary — at the shared ctor-argument reader, the scalar value-init arm and `new`; and a function template's explicit argument keeps its declarator (`f<int&>` bound T = int before: silent wrong type). Also 2ab96a707: a pure virtual DESTRUCTOR makes the class abstract and its vtable slots `__cxa_pure_virtual` (the abstract-class probes of sfinae8/9/12 could not even be emitted). **T4 (e42d37e5e) +2**: sizeof/alignof of void or a function type is ill-formed in C++ (after the type-id's declarator folds; `sizeof(void)` stays the GNU 1 in C), `T()` of an array type and an empty `T{}` of an array of unknown bound are refused. Remaining in the SFINAE plan: T4-full static_cast rules (need cv kept on captured template args), non-type default viability (Gap nontype_default_sfinae_viability), T5 access, T6 no-viable-operator. Ratchet `scripts/gxx_lane.sh` + `docs/parity/gxx-c++11-baseline.txt`, ledger row `gxx-c++11` promote=no — a ROADMAP GOAL, never a push or release gate (owner ruling). 🎯 **OWNER TARGET 2026-09-18: cut a MASTER RELEASE at 75%** = 1463/1950 — **REACHED 2026-09-20 (e42d37e5e)**; 75% is the TRIGGER, not the release (the release-tier lanes, the gcc-torture promote gate and the owner's own call still apply). **2026-09-20: the FOUR pre-existing container failures (testifconstexpr, testinvocable, testvecmembercopy_libcxx, testvecpb_libcxx) are SETTLED** (ebe9ac190 a partial spec's requires-clause folds in ITS declaring namespace; 3bdf58ee5 a function-template instantiation finds its product by the registrar's identity tag, not the overload set's last entry; ac5ec34c2 + 0ab598767 the three fulltest stages that had been dark behind the red suite since 2026-09-17: two static gates from the lambda-capture slice, and forest_bind_gate [strbind], where the forest flush had not followed two later live changes — a namespace-scope definition mangles, a constexpr scalar folds) — both were exposures of latent defects by correct fixes, found by a two-family bisect on the container; container fulltest 1529/0 with every post-suite stage running again. The develop SEAM is unblocked. ⚠⚠ OWNER PIVOT 2026-09-17: the lane's FIRST job is to be the self-host arc's REDUCER SOURCE. ⚠⚠ **AN ERROR MESSAGE IS NOT A FAMILY** (measured 2026-09-18): the "115 integer-constant-expression" family is FIVE unrelated families — 21 object member reads, 18 `Template<...>::value`, 18 POINTER-valued, 16 constexpr calls, 15 array bounds — and constexpr CONSTRUCTOR evaluation (now landed, `eeb715b4b`) converted just **1** of them; only 4 of 113 even reach the materializer. Classify by the failing CONSTRUCT before choosing a slice: the histogram that found the real lever took six minutes and paid 22x. That lever was the **31-test template-parameter-list** family, which WAS one gap — a non-type template parameter is a parameter-declaration ([temp.param]/1), so its head is followed by a DECLARATOR (`int (*p)()`, `T C::*M`, `Opt const& o`) and the list parser read only a bare NAME: **+22 in one slice**. Remaining mass of 556 failures: 39 undeclared identifier (CLASSIFIED 2026-09-18 — six constructs, NOT one: 12 unqualified lookup at instantiation (ADL/injected friend), 8 decltype in an unhandled position, 5 lambda body naming an enclosing-scope variable, 3 GNU vector builtins, 1 raw string literal, 11 singletons. ⚠ The two BIGGEST sub-groups are long tails, not gaps: a reducer shows plain ADL and a trailing-return `decltype(p + N)` already work, so those 20 fail one reason each. The lever inside this family was **user-defined literals** — absent entirely on the USE side, 7 first-error and 14 units containing an `operator""`; LANDED, +3), 23 no-matching-ctor, 21 constexpr object reads (NSDMI/bases/arrays-of-objects/addresses), 18 template values, 18 pointer constants, 16 constexpr calls, 15 identifier-after-type, 11 noexcept operand, **24 static assertion failed of which 11 are SFINAE** (WRONG ANSWERS, not parse refusals). ⚠ Family counting MUST normalize quoted text or a family splits into singletons. Cross-standard (`--gxx-dirs`, d61af6f42): C++14 196/417 47.0%, C++17 118/308 38.3%, C++20 336/653 51.5%, **cumulative C++17 1679/2675 = 62.8%**; CTAD is 33% of C++17 failures — design settled in [2026-09-18-ctad-implementation.md](2026-09-18-ctad-implementation.md), NOT on the C++11 path | [2026-09-04-cxx-conformance-lane.md](2026-09-04-cxx-conformance-lane.md) |
