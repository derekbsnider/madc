...
- **CIR baseline (2026-06-10):** **547 integration/unit pass / 0 fail / 0 timeout / 26 skip — ALL integration reds green** (testfstream, the last red, passes through real libstdc++ headers incl. real-header `std::to_string`/`std::stoi` via namespace function-template BODY instantiation);
...
| 1.3 | **CIR coverage — drive `cir_node` (MC11-IR) → c2mir → MIR to full parity** | ongoing | **Active — the parity-to-master gate** (547 pass / 0 fail / 0 timeout / 26 skip on 2026-06-10 — ZERO integration reds; the testfstream plan [2026-06-10-testfstream-alias-reference-plan.md](2026-06-10-testfstream-alias-reference-plan.md) executed in full; remaining parity work is the gcc-torture gap: 1567/1685 = 93.0% vs asmjit 97.6%, gap 78) | — |
