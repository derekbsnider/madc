...
- **CIR baseline (2026-06-10, post-v0.27.0 @ `8a897f8`):** **549 integration/unit pass / 0 fail / 0 timeout / 26 skip — ALL integration reds green** (new: `teststringplus_realhdr` + `testcstdio` — real-header `std::string a+b` via by-value FREE-operator returns bound mangled-direct, decl inits copy-elided into the variable; v0.27.0 had landed testfstream through real libstdc++ headers incl. real-header `std::to_string`/`std::stoi` via namespace function-template BODY instantiation);
...
| 1.3 | **CIR coverage — drive `cir_node` (MC11-IR) → c2mir → MIR to full parity** | ongoing | **Active — the parity-to-master gate** (549 pass / 0 fail / 0 timeout / 26 skip on 2026-06-10 @ `23027f7` — ZERO integration reds; real-header `std::string a+b` landed via by-value free-operator returns; remaining parity work is the gcc-torture gap: 1567/1685 = 93.0% vs asmjit 97.6%, gap 78) | — |
