...
| 2.11 | **Self-host: madc compiles its own C++11 source** (ultimate dogfood) | large | **IN PROGRESS (started 2026-09-17, owner go)** — the harness is LIVE: `scripts/selfhost_lane.sh` / `make -C src selfhost` runs every TU the Makefile links (72, flags derived by `make -n`) + 100 own-header wrapper TUs through `--emit=c11`, ratcheted over `docs/parity/selfhost-baseline.txt`; **53 of 172 units pass** (7 rt/*.c, 9 src TUs, 37 headers) after the first fixes: `thread_local`/`_Thread_local` (+ a c2mir block-scope fix), unnamed namespaces, `dynamic_cast<const T*>`, `typedef enum : T {}`, class `operator[]` element typing on reference/member receivers (+ a CIR block-scope struct-typedef tag fix), qualified class-scoped constants in constant expressions, `auto *p = new T`; next by yield: pointer-to-member-function type (29), `tuple_element<__i, const _Tp>` partial specialization (20), the `<regex>` recovery loop, tsubst deferred-construction packs (10) | failure-driven `selfhost` harness |
