...
| 8.7 | **REPL + Thonny-style teaching madcide** — `madc --repl` over ONE engine `ReplSession` (a new incremental-session core, NOT `eval_*`: one live Program, a MIR module per entry linked into one live context, hosted in a backend process), then a simple/teaching madcide profile consuming it (REPL panel, F5 = `%run`, Variables view). Behaviour follows Julia + IPython first, Clang-Repl second (owner decisions D1–D19) | Multi-phase | **STARTED 2026-09-25** on `feature/repl-teaching-ide-claude`: plan cross-referenced against the code + decisions (8a08cc164); D18 redeclaration prerequisites LANDED (bd3c56500 objects, a74fd49ba C functions). NEXT Phase 0: the complete/incomplete/invalid classifier, the persistent-session proof, rollback, result capture | [plan](madc-repl-thonny-plan-2026-09-24.md) (supersedes the [2026-09-21 plan](2026-09-21-repl-and-teaching-ide.md)'s staging) |
