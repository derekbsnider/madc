...
- **CIR baseline (2026-06-11, `feature/template-instantiation-claude` @ `92adc35`):** **561 integration/unit pass / 0 fail / 0 timeout / 18 skip** (template-instantiation batch: 2a fn-template empty-pack elision — std::stof/stod/stold; 2b-i `"pre" + s` exported mangled-direct; 2b-ii `a + "literal"` free-operator **BODY instantiation** + the char_traits explicit-spec instantiation-key fix — `std::char_traits<char>::length` silently folded to 0 before; previous 2026-06-11 develop baseline was 557/0/0/18 — eval leftovers B+A0+A: DSL string VALUE compares, MadValue/MadArray → one `madc::value`, mangled-direct `<ns_madc>` + user-call-site scope capture; libmadc in-process eval runs on CirJitSession);
