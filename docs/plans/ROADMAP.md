...
| 2.12 | **C++11 conformance measured by a third-party suite** — gcc's g++ testsuite (`g++.dg` + `g++.old-deja` `dg-do run` tests with no target clause or `target c++11`: ~2330) driven by the existing gcc-torture runner, ratcheted against a baseline | first measurement DONE | **MEASURED (2026-09-17)** — **1169/1950 in-scope (60%)** on g++.dg cpp0x+template compile-clean, `--std=c++11`; ratchet `scripts/gxx_lane.sh` + `docs/parity/gxx-c++11-baseline.txt` (781 baselined), ledger row `gxx-c++11` promote=no. A ROADMAP GOAL, never a push or release gate (owner ruling); the metric is class-(a) C++11 failures → 0. ⚠⚠ OWNER PIVOT 2026-09-17: the lane's FIRST job is to be the self-host arc's REDUCER SOURCE — gcc ships pre-reduced cases with oracles, so stop hand-building them from header failures. Top families: 124 integer-constant-expression, 84 `'<' after template`, 57 lambda (init-capture + capture list), 42 undeclared identifier; 2 SEGFAULTS (cpp0x/pr54318.C, template/friend7.C). **Diagnostic slice (2026-09-18, `2d0a215d1`):** all 19 recorded empty errors now report `static assertion failed`; underlying conformance failures remain, lane measurement unchanged | [2026-09-04-cxx-conformance-lane.md](2026-09-04-cxx-conformance-lane.md) |
