...
`feature/simd-vector-support-codex` is at `c69f4da`, not yet pinned by madc's
...
tests 2238`, the new checked-in exact vector copies passed GCC native plus
C2MIR `-ei` / `-eg`, and focused builtin-fp and one-lane unsigned `__int128`
