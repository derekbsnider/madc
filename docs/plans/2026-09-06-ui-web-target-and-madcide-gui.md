...
| Macs | The arm64 Mac (ssh `madc-mac`, macOS 15.3.2; WebKit.framework; CLT SDK MacOSX15.sdk) is **Jane's and in use — ssh only, no desktop access** (owner 2026-09-06): build/verify there, never a window test. Desktop (window) tests run on the **owner's x86 (Intel) MacBook**, which the owner controls: ssh alias `madc-mac-x86` (derek.snider@192.168.1.201; probed 2026-09-06: x86_64, macOS 15.7.4, console user logged in, CommandLineTools with Apple clang 17 + SDK MacOSX26.2.sdk, WebKit.framework present; no cmake / pkg-config / brew — the library builds by a one-line clang++ invocation there). |
