...
- **What is left of §41.3:**
  - **The Program rollback.** A refused entry's declarations, types, macros and includes stay. After a refused `int h(void) { … }`, `h` is declared but withheld, so a later `h()` is refused by name and a corrected `h` is a redefinition. cling and clang-repl roll the input back whole.
  - The rollback needs registration transactions that nest: `ClassRegistrationJournal` records only as the outermost journal, so an entry-wide one would disable every class-level rollback inside the entry. It also needs journals for macros, includes and namespaces.
...
D18 and the classifier are done. §41.2a slices 1, 2 and 2b are done: entries persist, an entry's statements run once, in source order, and they run under every standard (D3). The entry transaction's JIT half (§41.3) is done: a refused entry, whether its parse, its translation or its link refused it, leaves the live context as it was, with a diagnostic, and its definitions never come alive later. Next is slice 3, C++ vague linkage and every class member body through `session_defined`: its re-emissions are refused cleanly now, but they are still refused. Then the rest of §41.3, the Program rollback of a refused entry's declarations, which deletes `session_withheld`. Owner pause (2026-09-25): until the REPL makes real progress, defects found off its path go into `BUGS.md` instead of being fixed on the spot. The `fix-what-you-find.md` rule itself is unchanged.
