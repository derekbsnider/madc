...
`istreambuf_iterator<> expects 2 argument(s), got 1` (a real free-fn-template instantiation/overload
bug). 3-oracle the call FIRST (part (a) is the test, part (b) is madc). Alternatively the "operator->
on an object" pair (testsubscriptarrow/testvectorptr, "expression before '->' must be a pointer") may
be a cleaner singleton. The deep container chain (testvector/map/set/subscript*/template*/refreturn)
is the hardest — leave for last.
