...
enum basic_type {
  ...
  /* Integer types: the first should be BOOL and the last should be
     UINT128.  The order is important -- do not change it.  */
  TP_BOOL,
  TP_CHAR,
  TP_SCHAR,
  TP_UCHAR,
            ...
            if (i >= len || (t = VARR_GET (token_t, arg, i))->code != T_ID) {
              ...
              if (i != len)
                ...
                res = (strcmp (t->repr, ADD_OVERFLOW) == 0 || strcmp (t->repr, SUB_OVERFLOW) == 0
                       ...
                       || strcmp (t->repr, CONVERT_VECTOR) == 0 || strcmp (t->repr, SHUFFLE) == 0
                       || strcmp (t->repr, SHUFFLE_VECTOR) == 0
...
DA (post_expr_part) {
  ...
  for (;;) {
    if (MC (T_INCDEC, pos, code)) {
      ...
      code = N_CALL;
      list = new_node (c2m_ctx, N_LIST);
      if (op->code == N_ID && strcmp (op->u.s.s, CONVERT_VECTOR) == 0) {
        if (!C (')')) {
          P (assign_expr);
...
D (unary_expr) {
  ...
  if ((r = TRY (par_type_name)) != err_node) {
    ...
    return r;
  } else if (MP (T_REALPART, pos)) {
    code = N_REALPART;
    P (unary_expr);
    return new_pos_node1 (c2m_ctx, code, pos, r);
  } else if (MP (T_IMAGPART, pos)) {
    code = N_IMAGPART;
    P (unary_expr);
    return new_pos_node1 (c2m_ctx, code, pos, r);
...
static void parse_init (c2m_ctx_t c2m_ctx) {
  ...
  kw_add (c2m_ctx, "__imag", T_IMAGPART, 0);
...
static int signed_integer_type_p (const struct type *type) {
  if (standard_integer_type_p (type)) {
    ...
    return ((tp == TP_CHAR && char_is_signed_p ()) || tp == TP_SCHAR || tp == TP_SHORT
            || tp == TP_INT || tp == TP_LONG || tp == TP_LLONG || tp == TP_INT128);
  }
...
static int small_integer_vector_type_p (c2m_ctx_t c2m_ctx, struct type *type) {
  size_t size = raw_type_size (c2m_ctx, type);

  return type->mode == TM_VECTOR && size <= 4 && integer_type_p (type->u.vector_type->el_type);
}
...
static int supported_integer_vector_type_p (c2m_ctx_t c2m_ctx, struct type *type) {
  struct type *el_type;
...
static int v128_i32_vector_type_p (c2m_ctx_t c2m_ctx, struct type *type) {
  return v128_vector_type_p (c2m_ctx, type) && supported_integer_vector_type_p (c2m_ctx, type);
}
...
static int v128_i32_packed_add_sub_vector_type_p (c2m_ctx_t c2m_ctx, struct type *type) {
  mir_size_t lane_size;
...
static int v128_i8_packed_mul_vector_type_p (c2m_ctx_t c2m_ctx, struct type *type) {
  return v128_i32_vector_type_p (c2m_ctx, type) && vector_lane_size (c2m_ctx, type) == 1;
}
...
static int v128_i32_packed_scalar_shift_type_p (c2m_ctx_t c2m_ctx, struct type *vector_type,
  ...
  if (lane_size != 2 && lane_size != 4 && lane_size != 8) return FALSE;
  return integer_type_p (count_type) && !vector_type_p (count_type);
}

static int v128_i32_packed_scalar_shift_op_p (c2m_ctx_t c2m_ctx, node_code_t code,
                                              struct type *vector_type,
                                              struct type *count_type) {
  mir_size_t lane_size;

  if (!v128_i32_packed_scalar_shift_type_p (c2m_ctx, vector_type, count_type)) return FALSE;
  lane_size = vector_lane_size (c2m_ctx, vector_type);
  if (lane_size != 8) return TRUE;
  if (code == N_LSH || code == N_LSH_ASSIGN) return TRUE;
...
static int v128_i64_signed_scalar_rshift_op_p (c2m_ctx_t c2m_ctx, node_code_t code,
                                               struct type *vector_type,
                                               struct type *count_type) {
  if (code != N_RSH && code != N_RSH_ASSIGN) return FALSE;
  if (!v128_i32_packed_scalar_shift_type_p (c2m_ctx, vector_type, count_type)) return FALSE;
  return vector_lane_size (c2m_ctx, vector_type) == 8
...
static int int128_vector_type_p (c2m_ctx_t c2m_ctx, struct type *type) {
  return vector_type_p (type) && raw_type_size (c2m_ctx, type) == 16
         && type->u.vector_type->nel == 1 && vector_lane_size (c2m_ctx, type) == 16
         && int128_type_p (type->u.vector_type->el_type);
}
...
static struct type integer_promotion (const struct type *type) {
  ...
  init_type (&res);
  res.mode = TM_BASIC;
  if (type->mode == TM_BASIC && TP_LONG <= type->u.basic_type
      && type->u.basic_type <= TP_UINT128) {
...
static int scalar_fits_integer_vector_lane_p (c2m_ctx_t c2m_ctx, struct type *vector_type,
  ...
  if (!supported_integer_vector_type_p (c2m_ctx, vector_type) || !integer_type_p (scalar_type))
    return FALSE;
...
static int scalar_fits_int128_vector_lane_p (c2m_ctx_t c2m_ctx, struct type *vector_type,
                                             struct type *scalar_type) {
  return int128_vector_type_p (c2m_ctx, vector_type) && integer_type_p (scalar_type)
         && raw_type_size (c2m_ctx, scalar_type) <= 16;
}

static struct type *int128_bin_op_vector_type (c2m_ctx_t c2m_ctx, struct type *type1,
                                               struct type *type2) {
  if (int128_vector_type_p (c2m_ctx, type1) && compatible_types_p (type1, type2, TRUE))
    return type1;
  if (int128_vector_type_p (c2m_ctx, type1)
      && scalar_fits_int128_vector_lane_p (c2m_ctx, type1, type2))
    return type1;
  if (int128_vector_type_p (c2m_ctx, type2) && compatible_types_p (type2, type1, TRUE))
    return type2;
  if (int128_vector_type_p (c2m_ctx, type2)
      && scalar_fits_int128_vector_lane_p (c2m_ctx, type2, type1))
    return type2;
  return NULL;
}

static struct type *int128_shift_vector_type (c2m_ctx_t c2m_ctx, struct type *type1,
                                              struct type *type2) {
  if (int128_vector_type_p (c2m_ctx, type1)) {
    if ((int128_vector_type_p (c2m_ctx, type2) && compatible_types_p (type1, type2, TRUE))
        || integer_type_p (type2))
      return type1;
  }
  if (int128_vector_type_p (c2m_ctx, type2) && integer_type_p (type1)
      && raw_type_size (c2m_ctx, type1) <= 16)
    return type2;
  return NULL;
}

static struct type *integer_bin_op_vector_type (c2m_ctx_t c2m_ctx, struct type *type1,
...
static int scalar_fits_v128_float_lane_p (c2m_ctx_t c2m_ctx, struct type *vector_type,
                                          struct type *scalar_type) {
  return v128_float_vector_type_p (c2m_ctx, vector_type) && arithmetic_type_p (scalar_type);
}

static struct type *v128_float_bin_op_vector_type (c2m_ctx_t c2m_ctx, struct type *type1,
                                                   struct type *type2) {
  if (v128_float_vector_type_p (c2m_ctx, type1) && compatible_types_p (type1, type2, TRUE))
    return type1;
  if (v128_float_vector_type_p (c2m_ctx, type1)
      && scalar_fits_v128_float_lane_p (c2m_ctx, type1, type2))
    return type1;
  if (v128_float_vector_type_p (c2m_ctx, type2)
      && scalar_fits_v128_float_lane_p (c2m_ctx, type2, type1))
    return type2;
  return NULL;
}
...
static int same_size_vector_integer_cast_p (c2m_ctx_t c2m_ctx, struct type *to_type,
                                            struct type *from_type) {
  if (raw_type_size (c2m_ctx, to_type) != raw_type_size (c2m_ctx, from_type)) return FALSE;
  return ((vector_type_p (to_type) && integer_type_p (from_type))
          || (integer_type_p (to_type) && vector_type_p (from_type)));
}

static int same_size_vector_reinterpret_cast_p (c2m_ctx_t c2m_ctx, struct type *to_type,
                                                struct type *from_type) {
  return same_size_vector_cast_p (c2m_ctx, to_type, from_type)
         || same_size_vector_integer_cast_p (c2m_ctx, to_type, from_type);
}
...
static int shufflevector_type_p (c2m_ctx_t c2m_ctx, struct type *res_type, struct type *type1,
                                 struct type *type2, size_t n_el) {
  if (type1->mode != TM_VECTOR || type2->mode != TM_VECTOR
      || !compatible_types_p (type1->u.vector_type->el_type, type2->u.vector_type->el_type, TRUE))
    return FALSE;
  return res_type->mode == TM_VECTOR
         && compatible_types_p (res_type->u.vector_type->el_type, type1->u.vector_type->el_type,
                                TRUE)
         && vector_el_count (c2m_ctx, res_type) == (mir_size_t) n_el
         && supported_vector_lane_type_p (c2m_ctx, type1->u.vector_type->el_type);
}

static int shuffle_type_p (c2m_ctx_t c2m_ctx, struct type *type1, struct type *type2,
                           struct type *mask_type) {
  struct type *mask_el_type;
  mir_size_t mask_lane_size;

  if (type1->mode != TM_VECTOR || mask_type->mode != TM_VECTOR) return FALSE;
  if (type2 != NULL && (type2->mode != TM_VECTOR || !compatible_types_p (type1, type2, TRUE)))
    return FALSE;
  mask_el_type = mask_type->u.vector_type->el_type;
  mask_lane_size = raw_type_size (c2m_ctx, mask_el_type);
  return supported_vector_lane_type_p (c2m_ctx, type1->u.vector_type->el_type)
...
static int basic_type_align (enum basic_type bt) {
  ...
  if (bt == TP_INT128 || bt == TP_UINT128) return 16;
...
static mir_ullong int128_const_low (struct expr *e) {
  assert (int128_type_p (e->type));
  return signed_integer_type_p (e->type) ? (mir_ullong) e->c.i_val : e->c.u_val;
}

static void set_int128_const_bits (struct expr *e, struct type *type, mir_ullong low,
                                   mir_ullong high) {
  assert (int128_type_p (type));
  if (signed_integer_type_p (type))
    e->c.i_val = (mir_llong) low;
  else
    e->c.u_val = low;
  e->c_u_hi_val = high;
}

/* Portable 128-bit constant arithmetic on (low, high) u64 pairs — the host
   compiler need not provide __int128.  Two's-complement wraparound semantics
   throughout (signedness only matters for rsh/div/mod, passed explicitly). */
static void int128_pair_add (mir_ullong l1, mir_ullong h1, mir_ullong l2, mir_ullong h2,
                             mir_ullong *lr, mir_ullong *hr) {
  *lr = l1 + l2;
  *hr = h1 + h2 + (*lr < l1);
}

static void int128_pair_neg (mir_ullong l, mir_ullong h, mir_ullong *lr, mir_ullong *hr) {
  *lr = ~l + 1;
  *hr = ~h + (*lr == 0);
}

static void int128_pair_sub (mir_ullong l1, mir_ullong h1, mir_ullong l2, mir_ullong h2,
                             mir_ullong *lr, mir_ullong *hr) {
  *hr = h1 - h2 - (l1 < l2);
  *lr = l1 - l2;
}

static void int128_pair_mul (mir_ullong l1, mir_ullong h1, mir_ullong l2, mir_ullong h2,
                             mir_ullong *lr, mir_ullong *hr) {
  mir_ullong a0 = l1 & 0xffffffffu, a1 = l1 >> 32;
  mir_ullong b0 = l2 & 0xffffffffu, b1 = l2 >> 32;
  mir_ullong p0 = a0 * b0, p1 = a0 * b1, p2 = a1 * b0, p3 = a1 * b1;
  mir_ullong mid = p1 + p2, carry_mid = mid < p1;
  mir_ullong low = p0 + (mid << 32);

  *hr = p3 + (mid >> 32) + (carry_mid << 32) + (low < p0) + l1 * h2 + h1 * l2;
  *lr = low;
}

static void int128_pair_lsh (mir_ullong l, mir_ullong h, unsigned sh, mir_ullong *lr,
                             mir_ullong *hr) {
  if (sh == 0) {
    *lr = l, *hr = h;
  } else if (sh < 64) {
...
static void int128_pair_rsh (mir_ullong l, mir_ullong h, unsigned sh, int signed_p,
                             mir_ullong *lr, mir_ullong *hr) {
  mir_ullong fill = signed_p && (h >> 63) ? ~(mir_ullong) 0 : 0;

  if (sh == 0) {
    *lr = l, *hr = h;
  } else if (sh < 64) {
    *lr = (l >> sh) | (h << (64 - sh));
    *hr = signed_p ? (mir_ullong) ((mir_llong) h >> sh) : h >> sh;
  } else if (sh < 128) {
    *lr = signed_p ? (mir_ullong) ((mir_llong) h >> (sh - 64)) : h >> (sh - 64);
    *hr = fill;
  } else {
    *lr = *hr = fill;
  }
...
static int int128_pair_ult (mir_ullong l1, mir_ullong h1, mir_ullong l2, mir_ullong h2) {
  return h1 < h2 || (h1 == h2 && l1 < l2);
}

/* Unsigned 128-bit long division (shift-subtract). divisor must be nonzero. */
static void int128_pair_udivmod (mir_ullong l1, mir_ullong h1, mir_ullong l2, mir_ullong h2,
                                 mir_ullong *ql, mir_ullong *qh, mir_ullong *rl,
                                 mir_ullong *rh) {
  mir_ullong quol = 0, quoh = 0, reml = 0, remh = 0;
  int i;

  for (i = 127; i >= 0; i--) {
    mir_ullong bit = i >= 64 ? (h1 >> (i - 64)) & 1 : (l1 >> i) & 1;

    remh = (remh << 1) | (reml >> 63);
    reml = (reml << 1) | bit;
    if (!int128_pair_ult (reml, remh, l2, h2)) {
      int128_pair_sub (reml, remh, l2, h2, &reml, &remh);
      if (i >= 64)
        quoh |= (mir_ullong) 1 << (i - 64);
      else
        quol |= (mir_ullong) 1 << i;
    }
  }
  *ql = quol, *qh = quoh, *rl = reml, *rh = remh;
}

/* Signed/unsigned 128-bit constant div/mod with C truncation semantics. */
static void int128_pair_divmod (int mod_p, int signed_p, mir_ullong l1, mir_ullong h1,
                                mir_ullong l2, mir_ullong h2, mir_ullong *lr,
                                mir_ullong *hr) {
  int neg1 = signed_p && (h1 >> 63), neg2 = signed_p && (h2 >> 63);
  mir_ullong ql, qh, rl, rh;

  if (neg1) int128_pair_neg (l1, h1, &l1, &h1);
  if (neg2) int128_pair_neg (l2, h2, &l2, &h2);
  int128_pair_udivmod (l1, h1, l2, h2, &ql, &qh, &rl, &rh);
  if (mod_p) {
    if (neg1) int128_pair_neg (rl, rh, &rl, &rh); /* remainder takes the dividend's sign */
    *lr = rl, *hr = rh;
  } else {
    if (neg1 != neg2) int128_pair_neg (ql, qh, &ql, &qh);
    *lr = ql, *hr = qh;
  }
}
...
static int null_const_p (struct expr *expr, struct type *type) {
  return ((integer_type_p (type) && expr->const_p
           && (!int128_type_p (type)
               ? expr->c.u_val == 0
               : int128_const_low (expr) == 0 && expr->c_u_hi_val == 0))
...
#define BASIC_TO_CONV(cast, mto)                                \
  ...
  if (to->mode == TM_BASIC && int128_type_p (to)) {
    mir_ullong low = 0, high = 0;

    if (from->mode == TM_PTR) {
      low = from_e->c.u_val;
    } else if (from->mode == TM_BASIC && int128_type_p (from)) {
      low = int128_const_low (from_e);
      high = from_e->c_u_hi_val;
    } else {
      switch (from->u.basic_type) {
      case TP_BOOL:
      case TP_UCHAR:
      ...
      case TP_UINT:
      case TP_ULONG:
      case TP_ULLONG: low = from_e->c.u_val; break;
      case TP_CHAR:
        if (!char_is_signed_p ()) {
          low = from_e->c.u_val;
          break;
      ...
      case TP_LONG:
      case TP_LLONG:
        low = (mir_ullong) from_e->c.i_val;
        high = from_e->c.i_val < 0 ? MIR_ULLONG_MAX : 0;
        break;
      case TP_FLOAT:
      case TP_DOUBLE:
      case TP_LDOUBLE: {
        /* Full-width float -> int128 constant conversion (the plain 64-bit
           cast dropped everything above 2^64). */
        mir_ldouble d = from_e->c.d_val;
        int neg_p = d < 0;
        mir_ldouble ad = neg_p ? -d : d;
        mir_ldouble two64 = 18446744073709551616.0L; /* 2^64 */

        high = (mir_ullong) (ad / two64);
        low = (mir_ullong) (ad - (mir_ldouble) high * two64);
        if (neg_p) {
          low = ~low + 1;
          high = ~high + (low == 0);
        }
    ...
    set_int128_const_bits (to_e, to, low, high);
  } else if (from->mode == TM_BASIC && int128_type_p (from)) {
    mir_ullong low = int128_const_low (from_e);
    ...
    if (to->mode == TM_PTR) {
      ...
      case TP_ULONG: to_e->c.u_val = (mir_ulong) low; break;
      case TP_ULLONG: to_e->c.u_val = (mir_ullong) low; break;
      case TP_CHAR:
        if (char_is_signed_p ())
          to_e->c.i_val = (mir_char) low;
        else
      ...
      case TP_LLONG: to_e->c.i_val = (mir_llong) low; break;
      case TP_FLOAT:
      case TP_DOUBLE:
      case TP_LDOUBLE:
      case TP_CFLOAT:
      case TP_CDOUBLE:
      case TP_CLDOUBLE: {
        /* Full-width int128 -> float constant conversion (the plain low-half
           cast dropped everything above 2^64). */
        mir_ullong high = from_e->c_u_hi_val, alow = low, ahigh = high;
        int neg_p = signed_integer_type_p (from) && (high >> 63);
        mir_ldouble two64 = 18446744073709551616.0L; /* 2^64 */
        mir_ldouble ad;
        ...
        ad = (mir_ldouble) ahigh * two64 + (mir_ldouble) alow;
        if (neg_p) ad = -ad;
        to_e->c.d_val = to->u.basic_type == TP_FLOAT || to->u.basic_type == TP_CFLOAT
                          ? (mir_float) ad
                          : to->u.basic_type == TP_DOUBLE || to->u.basic_type == TP_CDOUBLE
                              ? (mir_double) ad
                              : ad;
        if (to->u.basic_type == TP_CFLOAT || to->u.basic_type == TP_CDOUBLE
            || to->u.basic_type == TP_CLDOUBLE)
          to_e->c_im_val = 0.0L;
        break;
      }
      default: assert (FALSE);
  ...
  } else if (to->mode == from->mode && (from->mode == TM_PTR || from->mode == TM_ENUM)) {
    to_e->c = from_e->c;
  ...
  for (node_t n = NL_HEAD (r->u.ops); n != NULL; n = NL_NEXT (n)) switch (n->code) {
    ...
    case N_INT128:
      ...
      if (type->mode != TM_BASIC || type->u.basic_type != TP_UNDEF)
        ...
        error (c2m_ctx, POS (n), "__int128 with _Complex");
      else
...
static int check_const_addr_p (c2m_ctx_t c2m_ctx, node_t r, node_t *base, mir_llong *offset,
  ...
  case N_CAST:
    ...
    if (decl_spec->type->mode != TM_PTR && !integer_type_p (decl_spec->type)) return FALSE;
...
/* Scan one attribute list (a declarator's attrs slot OR a declaration-specs
   list, which holds N_ATTR nodes mixed with spec nodes) for
   __attribute__((cleanup(fn))) and record it on the decl + its block scope
   (create_decl owns the automatic-variable-only guard). */
static void scan_cleanup_attrs (c2m_ctx_t c2m_ctx, decl_t decl, node_t decl_node, node_t scope,
                                node_t attrs) {
  ...
  for (node_t a = NL_HEAD (attrs->u.ops); a != NULL; a = NL_NEXT (a)) {
    ...
    if (aname == NULL || aname->code != N_ID || strcmp (aname->u.s.s, "cleanup") != 0) continue;
...
static void create_decl (c2m_ctx_t c2m_ctx, node_t scope, node_t decl_node,
  ...
  /* __attribute__((cleanup(fn))): record the cleanup function on the decl and
     ...
     The attribute is legal in BOTH positions -- declarator suffix (the
     N_SPEC_DECL attrs slot) and declaration-specs prefix (N_ATTR nodes
     flattened into the specs list, applying to every declarator) -- so scan
     both, the same per-position pairing vector attrs use. */
  if (decl_node->code == N_SPEC_DECL && declarator->code == N_DECL && scope != top_scope
    ...
    node_t specs = NL_HEAD (decl_node->u.ops);
    if (specs != NULL && specs->code == N_SHARE) specs = NL_HEAD (specs->u.ops);
    scan_cleanup_attrs (c2m_ctx, decl, decl_node, scope, specs);
    scan_cleanup_attrs (c2m_ctx, decl, decl_node, scope, NL_EL (decl_node->u.ops, 2));
  }
...
static struct expr *check_assign_op (c2m_ctx_t c2m_ctx, node_t r, struct expr *e1, struct expr *e2,
    ...
    if ((tt = integer_bin_op_vector_type (c2m_ctx, t1, t2, e1, e2)) != NULL) {
      ...
      *e->type = *tt;
      ...
      if (e1->const_p && e2->const_p) {
        ...
        e->const_p = TRUE;
        if (int128_type_p (&t)) {
          mir_ullong low1 = int128_const_low (e1), low2 = int128_const_low (e2);

          set_int128_const_bits (e, &t,
                                 r->code == N_AND  ? low1 & low2
                                 : r->code == N_OR ? low1 | low2
                                                   : low1 ^ low2,
                                 r->code == N_AND  ? e1->c_u_hi_val & e2->c_u_hi_val
                                 : r->code == N_OR ? e1->c_u_hi_val | e2->c_u_hi_val
                                                   : e1->c_u_hi_val ^ e2->c_u_hi_val);
  ...
  case N_RSH_ASSIGN:
    ...
    if ((tt = integer_shift_vector_type (c2m_ctx, t1, t2, e1, e2)) != NULL) {
      *e->type = *tt;
    } else if ((tt = int128_shift_vector_type (c2m_ctx, t1, t2)) != NULL) {
      *e->type = *tt;
    } else if (!integer_type_p (t1) || !integer_type_p (t2)) {
      ...
      if (e1->const_p && e2->const_p) {
        ...
        if (int128_type_p (&t)) {
          unsigned count
            = (unsigned) (int128_type_p (&rt) ? int128_const_low (e2) : e2->c.u_val) & 0xff;
          mir_ullong lr, hr;

          if (r->code == N_LSH || r->code == N_LSH_ASSIGN)
            int128_pair_lsh (int128_const_low (e1), e1->c_u_hi_val, count, &lr, &hr);
          else
            int128_pair_rsh (int128_const_low (e1), e1->c_u_hi_val, count,
                             signed_integer_type_p (&t), &lr, &hr);
          set_int128_const_bits (e, &t, lr, hr);
        } else if (signed_integer_type_p (&t)) {
  ...
  case N_SUB_ASSIGN: {
    ...
    if (arith_or_complex_type_p (t1) && arith_or_complex_type_p (t2)) {
      ...
      if (e1->const_p && e2->const_p) {
        ...
        if (complex_type_p (&t)) {
          ...
          if (floating_type_p (&t))
            ...
            if (add_p)
              int128_pair_add (int128_const_low (e1), e1->c_u_hi_val, int128_const_low (e2),
                               e2->c_u_hi_val, &lr, &hr);
            else
              int128_pair_sub (int128_const_low (e1), e1->c_u_hi_val, int128_const_low (e2),
                               e2->c_u_hi_val, &lr, &hr);
            set_int128_const_bits (e, &t, lr, hr);
          } else if (signed_integer_type_p (&t))
      ...
      *e->type = *tt;
    } else if (add_sub_vector_p && (tt = int128_bin_op_vector_type (c2m_ctx, t1, t2)) != NULL) {
      *e->type = *tt;
    } else if (add_sub_vector_p
  ...
  case N_MOD_ASSIGN:
    ...
    if ((tt = integer_bin_op_vector_type (c2m_ctx, t1, t2, e1, e2)) != NULL) {
      ...
      *e->type = *tt;
               ...
               && (tt = v128_float_bin_op_vector_type (c2m_ctx, t1, t2)) != NULL) {
      *e->type = *tt;
    } else if (r->code == N_MOD && (!integer_type_p (t1) || !integer_type_p (t2))) {
      error (c2m_ctx, POS (r), "invalid operand types of %%");
      ...
      if (e1->const_p && e2->const_p) {
        ...
        if (complex_type_p (&t)) {
          ...
          if (int128_type_p (&t)) {
            ...
            if (r->code == N_MUL) {
              int128_pair_mul (int128_const_low (e1), e1->c_u_hi_val, int128_const_low (e2),
                               e2->c_u_hi_val, &lr, &hr);
              set_int128_const_bits (e, &t, lr, hr);
            } else if (int128_const_low (e2) == 0 && e2->c_u_hi_val == 0) {
              set_int128_const_bits (e, &t, 0, 0);
              error (c2m_ctx, POS (r), "Division by zero");
            } else {
              int128_pair_divmod (r->code == N_MOD || r->code == N_MOD_ASSIGN,
                                  signed_integer_type_p (&t), int128_const_low (e1),
                                  e1->c_u_hi_val, int128_const_low (e2), e2->c_u_hi_val, &lr,
                                  &hr);
              set_int128_const_bits (e, &t, lr, hr);
            }
...
static void check (c2m_ctx_t c2m_ctx, node_t r, node_t context) {
  ...
  case N_OROR:
    ...
    if (!scalar_type_p (t1) || !scalar_type_p (t2)) {
      ...
      if (floating_type_p (t1))
        v = e1->c.d_val != 0.0;
      else if (int128_type_p (t1))
        v = int128_const_low (e1) != 0 || e1->c_u_hi_val != 0;
      ...
      if (v && r->code == N_OROR) {
        ...
        if (floating_type_p (t2))
          v = e2->c.d_val != 0.0;
        else if (int128_type_p (t2))
          v = int128_const_low (e2) != 0 || e2->c_u_hi_val != 0;
  ...
  case N_GE:
    ...
    e->type->u.basic_type = TP_INT;
    {
      struct type *cmp_type = integer_cmp_vector_type (c2m_ctx, r->code, t1, t2, e1, e2);

      if (cmp_type == NULL) cmp_type = v128_float_cmp_vector_type (c2m_ctx, t1, t2);
      if (cmp_type != NULL) {
        *e->type = *cmp_type;
        break;
      }
    ...
    if ((r->code == N_EQ || r->code == N_NE)
      ...
      if (e1->const_p && e2->const_p) {
        ...
        if (floating_type_p (&t))
          ...
          mir_ullong low1 = int128_const_low (e1), low2 = int128_const_low (e2);
          mir_ullong high1 = e1->c_u_hi_val, high2 = e2->c_u_hi_val;

          if (r->code == N_EQ) {
            e->c.i_val = low1 == low2 && high1 == high2;
          } else if (r->code == N_NE) {
            e->c.i_val = low1 != low2 || high1 != high2;
          } else if (signed_integer_type_p (&t)) {
            mir_llong shigh1 = (mir_llong) high1, shigh2 = (mir_llong) high2;

            e->c.i_val = (r->code == N_LT   ? shigh1 < shigh2 || (shigh1 == shigh2 && low1 < low2)
                          : r->code == N_LE ? shigh1 < shigh2 || (shigh1 == shigh2 && low1 <= low2)
                          : r->code == N_GT ? shigh1 > shigh2 || (shigh1 == shigh2 && low1 > low2)
                                            : shigh1 > shigh2
                                                || (shigh1 == shigh2 && low1 >= low2));
          } else {
            e->c.i_val = (r->code == N_LT   ? high1 < high2 || (high1 == high2 && low1 < low2)
                          : r->code == N_LE ? high1 < high2 || (high1 == high2 && low1 <= low2)
                          : r->code == N_GT ? high1 > high2 || (high1 == high2 && low1 > low2)
                                            : high1 > high2
  ...
  case N_NOT:
    ...
    e->type->mode = TM_BASIC;
    e->type->u.basic_type = TP_INT;
    if (r->code == N_BITWISE_NOT
        && (supported_integer_vector_type_p (c2m_ctx, t1)
            || int128_vector_type_p (c2m_ctx, t1))) {
      *e->type = *t1;
      ...
      if (e1->const_p) {
        ...
        e->const_p = TRUE;
        if (int128_type_p (&t))
          set_int128_const_bits (e, &t, ~int128_const_low (e1), ~e1->c_u_hi_val);
        else if (signed_integer_type_p (&t))
      ...
      if (floating_type_p (t1))
        ...
        e->c.i_val = int128_const_low (e1) == 0 && e1->c_u_hi_val == 0;
      else if (signed_integer_type_p (t1))
  ...
  case N_IMAGPART:
    ...
    e = create_expr (c2m_ctx, r);
    e->type->mode = TM_BASIC;
    if (!complex_type_p (t1)) {
      if (arithmetic_type_p (t1)) {
        /* __real__ on a real type is identity; __imag__ gives 0. */
        e->type->u.basic_type = t1->u.basic_type;
      } else {
        error (c2m_ctx, POS (r), "__real__/__imag__ operand should be of a complex or arithmetic type");
        e->type->u.basic_type = TP_DOUBLE;
      }
    } else {
      e->type->u.basic_type = complex_component_type (t1->u.basic_type);
      e->u.lvalue_node = op1;  /* it is an lvalue */
    }
    break;
  case N_INC:
  case N_DEC:
  ...
  case N_SUB:
    if (NL_NEXT (NL_HEAD (r->u.ops)) == NULL) { /* unary */
      ...
      e->type->mode = TM_BASIC;
      e->type->u.basic_type = TP_INT;
      if (supported_integer_vector_type_p (c2m_ctx, t1)
          || int128_vector_type_p (c2m_ctx, t1) || v128_float_vector_type_p (c2m_ctx, t1)) {
        *e->type = *t1;
        ...
        if (floating_type_p (t1)) {
          ...
          if (e1->const_p) {
            convert_value (e1, &t);
            if (int128_type_p (&t)) {
              mir_ullong lr = int128_const_low (e1), hr = e1->c_u_hi_val;

              if (r->code == N_SUB) int128_pair_neg (lr, hr, &lr, &hr);
              set_int128_const_bits (e, &t, lr, hr);
            } else if (signed_integer_type_p (&t))
  ...
  case N_CAST: {
    ...
    *e->type = *decl_spec->type;
    ...
    vector_cast_p = same_size_vector_reinterpret_cast_p (c2m_ctx, decl_spec->type, t2);
    ...
    } else if (e2->const_p && !void_p && !vector_cast_p) {
      e->const_p = TRUE;
      cast_value (e, e2, decl_spec->type);
    }
    if (vector_cast_p) e->const_p = FALSE;
    break;
  }
  case N_COMPOUND_LITERAL: {
    node_t list, n;
    ...
    if (builtin_call_p) {
      ...
      if (builtin_call_p
        ...
        if (vectorelements_p) {
          ...
          e2 = arg->attr;
          t2 = e2->type;
          if (type_arg->code != N_TYPE) {
            error (c2m_ctx, POS (type_arg), "2nd argument of %s should be a vector type",
                   CONVERT_VECTOR);
          } else {
            to_decl_spec = type_arg->attr;
          ...
          size_t n_args = NL_LENGTH (arg_list->u.ops);
          node_t mask_arg;
          struct expr *mask_expr;
          struct type *mask_type;
          ...
          mask_expr = mask_arg->attr;
          mask_type = mask_expr->type;
          ret_type = t1;
          if (t1->mode != TM_VECTOR || (t2 != NULL && t2->mode != TM_VECTOR)
              || mask_type->mode != TM_VECTOR) {
            error (c2m_ctx, POS (r), "arguments of %s should be vectors", SHUFFLE);
          } else if (!shuffle_type_p (c2m_ctx, t1, t2, mask_type)) {
            error (c2m_ctx, POS (r), "unsupported vector types in %s", SHUFFLE);
          }
        } else if (shufflevector_p) {
          size_t n_args = NL_LENGTH (arg_list->u.ops);
          mir_size_t src1_n_el, src2_n_el, max_index;
          mir_size_t lane_size, res_size;
          ...
          t2 = e2->type;
          ret_type = t1;
          if (t1->mode != TM_VECTOR || t2->mode != TM_VECTOR) {
            error (c2m_ctx, POS (r), "first two arguments of %s should be vectors",
                   SHUFFLE_VECTOR);
          } else {
            ...
            if (!shufflevector_type_p (c2m_ctx, ret_type, t1, t2, n_args - 2)) {
              ...
              src1_n_el = vector_el_count (c2m_ctx, t1);
              src2_n_el = vector_el_count (c2m_ctx, t2);
              max_index = src1_n_el + src2_n_el;
              for (arg = NL_EL (arg_list->u.ops, 2); arg != NULL; arg = NL_NEXT (arg)) {
                struct expr *idx_expr = arg->attr;
                mir_llong idx;

                if (!idx_expr->const_p || !integer_type_p (idx_expr->type)) {
                  error (c2m_ctx, POS (arg), "shuffle index should be an integer constant");
                  continue;
                }
                idx = signed_integer_type_p (idx_expr->type) ? idx_expr->c.i_val
                                                             : (mir_llong) idx_expr->c.u_val;
                if (idx < -1 || idx >= (mir_llong) max_index)
                  error (c2m_ctx, POS (arg), "shuffle index is out of range");
              }
            }
          }
        } else if (memcmp_p) {
...
struct gen_ctx {
  ...
  /* int128 <-> floating conversion helpers: protos per float width (F/D/LD),
     import items per libgcc helper name (2 signedness x 3 widths x 2 dirs). */
  MIR_item_t int128_to_float_protos[3], float_to_int128_protos[3];
  MIR_item_t int128_conv_items[12];
  MIR_item_t int128_ovfl_proto, int128_ovfl_items[6];
...
#define int128_to_float_protos gen_ctx->int128_to_float_protos
#define float_to_int128_protos gen_ctx->float_to_int128_protos
#define int128_conv_items gen_ctx->int128_conv_items
#define int128_ovfl_proto gen_ctx->int128_ovfl_proto
#define int128_ovfl_items gen_ctx->int128_ovfl_items
...
static MIR_type_t get_mir_type (c2m_ctx_t c2m_ctx, struct type *type) {
  ...
  if (int128_type_p (type)) return MIR_T_UNDEF;
...
/* A union access keeps the union's alias class while a pointer-deref access to the same
   memory carries the member type's own class, so the two classes must be declared
   conflicting ([class.union] type punning through unions is valid C).  Walk the union's
   value-embedded member tree (pointers are leaves) and register a conflict between the
   union class and every distinct member access class. */
...
static int push_const_val (c2m_ctx_t c2m_ctx, node_t r, op_t *res) {
  ...
  if (int128_type_p (e->type)) return FALSE;
...
static op_t vector_temp (c2m_ctx_t c2m_ctx, struct type *type) {
  MIR_context_t ctx = c2m_ctx->ctx;
  mir_size_t size = raw_type_size (c2m_ctx, type);
  MIR_type_t mem_type = size == 16 ? MIR_T_V128 : MIR_T_I8;
  op_t addr = get_new_temp (c2m_ctx, MIR_T_I64);

  emit2 (c2m_ctx, MIR_ALLOCA, addr.mir_op, MIR_new_int_op (ctx, size));
  return new_op (NULL, MIR_new_mem_op (ctx, mem_type, 0, addr.mir_op.u.reg, 0, 1));
}

static op_t scalar_to_v128_lane (c2m_ctx_t c2m_ctx, op_t scalar, struct type *scalar_type,
                                 struct type *vector_type) {
  ...
  assert (supported_vector_lane_type_p (c2m_ctx, vector_type->u.vector_type->el_type)
          && arithmetic_type_p (scalar_type));
...
static op_t scalar_to_integer_vector (c2m_ctx_t c2m_ctx, op_t scalar, struct type *scalar_type,
                                      struct type *vector_type) {
  assert (supported_integer_vector_lane_size_p (vector_lane_size (c2m_ctx, vector_type))
          && integer_type_p (scalar_type));
  return scalar_to_v128_lane (c2m_ctx, scalar, scalar_type, vector_type);
}
...
static op_t prepare_v128_i32_operand (c2m_ctx_t c2m_ctx, op_t op, struct type *op_type,
  ...
  if (!v128_i32_vector_type_p (c2m_ctx, op_type))
    op = scalar_to_integer_vector (c2m_ctx, op, op_type, vector_type);
...
static MIR_insn_code_t get_integer_vector_bitwise_insn_code (c2m_ctx_t c2m_ctx,
                                                             node_code_t code,
                                                             struct type *vector_type) {
  MIR_type_t lane_type = get_mir_type (c2m_ctx, vector_type->u.vector_type->el_type);
  MIR_type_t op_type = promote_mir_int_type (lane_type);

  assert (supported_integer_vector_type_p (c2m_ctx, vector_type));
  switch (code) {
  case N_AND:
  case N_AND_ASSIGN: return op_type == MIR_T_I64 || op_type == MIR_T_U64 ? MIR_AND : MIR_ANDS;
  case N_OR:
  case N_OR_ASSIGN: return op_type == MIR_T_I64 || op_type == MIR_T_U64 ? MIR_OR : MIR_ORS;
  case N_XOR:
  case N_XOR_ASSIGN: return op_type == MIR_T_I64 || op_type == MIR_T_U64 ? MIR_XOR : MIR_XORS;
  default: assert (FALSE); return MIR_INSN_BOUND;
  }
}

static MIR_insn_code_t get_v128_i32_packed_add_sub_insn_code (c2m_ctx_t c2m_ctx,
                                                              node_code_t code,
                                                              struct type *vector_type) {
  mir_size_t lane_size = vector_lane_size (c2m_ctx, vector_type);

  assert (v128_i32_packed_add_sub_vector_type_p (c2m_ctx, vector_type));
  switch (code) {
  case N_ADD:
  case N_ADD_ASSIGN:
...
static op_t materialize_vector_operand (c2m_ctx_t c2m_ctx, op_t op, struct type *vector_type) {
  op_t res;

  assert (vector_type_p (vector_type));
  res = vector_temp (c2m_ctx, vector_type);
  if (op.mir_op.mode == MIR_OP_REG) {
    assert (v128_vector_type_p (c2m_ctx, vector_type));
    store_v128 (c2m_ctx, res, op);
  } else {
    assert (op.mir_op.mode == MIR_OP_MEM);
    block_move (c2m_ctx, res, op, raw_type_size (c2m_ctx, vector_type));
  }
  return res;
}

static op_t materialize_v128_operand (c2m_ctx_t c2m_ctx, op_t op, struct type *vector_type) {
  op_t res;

  assert (v128_vector_type_p (c2m_ctx, vector_type));
  if (op.mir_op.mode != MIR_OP_REG) op = force_v128_reg (c2m_ctx, op);
  res = materialize_vector_operand (c2m_ctx, op, vector_type);
  return res;
}

static op_t materialize_integer_vector_operand (c2m_ctx_t c2m_ctx, op_t op,
                                                struct type *op_type,
                                                struct type *vector_type) {
  if (!supported_integer_vector_type_p (c2m_ctx, op_type))
    return scalar_to_integer_vector (c2m_ctx, op, op_type, vector_type);
...
static op_t vector_lane_op (op_t vector, MIR_type_t lane_type, mir_size_t offset) {
  ...
  assert (lane.mir_op.mode == MIR_OP_MEM);
  lane.mir_op.u.mem.type = lane_type;
  lane.mir_op.u.mem.disp += offset;
  return lane;
}

static op_t v128_i32_lane_op (op_t vector, MIR_type_t lane_type, mir_size_t offset) {
  return vector_lane_op (vector, lane_type, offset);
}
...
static op_t const_integer_vector (c2m_ctx_t c2m_ctx, struct type *vector_type, mir_int val) {
  MIR_context_t ctx = c2m_ctx->ctx;
...
static op_t int128_temp (c2m_ctx_t c2m_ctx) {
  MIR_context_t ctx = c2m_ctx->ctx;
  op_t addr = get_new_temp (c2m_ctx, MIR_T_I64);

  emit2 (c2m_ctx, MIR_ALLOCA, addr.mir_op, MIR_new_int_op (ctx, 16));
...
static void store_int128_halves (c2m_ctx_t c2m_ctx, op_t dest, op_t low, op_t high) {
  op_t low_dest = int128_half_op (c2m_ctx, dest, MIR_T_U64, 0);
  op_t high_dest = int128_half_op (c2m_ctx, dest, MIR_T_U64, 8);

  emit_scalar_assign (c2m_ctx, low_dest, &low, MIR_T_U64, FALSE);
  emit_scalar_assign (c2m_ctx, high_dest, &high, MIR_T_U64, FALSE);
}

static op_t const_int128_mem (c2m_ctx_t c2m_ctx, struct expr *expr) {
  MIR_context_t ctx = c2m_ctx->ctx;
  op_t res = int128_temp (c2m_ctx);
  op_t low = new_op (NULL, MIR_new_uint_op (ctx, int128_const_low (expr)));
  op_t high = new_op (NULL, MIR_new_uint_op (ctx, expr->c_u_hi_val));

  assert (expr->const_p && int128_type_p (expr->type));
  store_int128_halves (c2m_ctx, res, low, high);
  return res;
}

static op_t float_to_int128_mem (c2m_ctx_t c2m_ctx, op_t fval, MIR_type_t ft, int signed_p);

static op_t scalar_to_int128_mem (c2m_ctx_t c2m_ctx, op_t scalar, struct type *scalar_type) {
  MIR_context_t ctx = c2m_ctx->ctx;
  op_t res, low, high;

  if (floating_type_p (scalar_type))
    /* Implicit float -> __int128: the signed helper.  (Negative-to-unsigned
       is UB, so the signed choice only diverges on UB inputs; an EXPLICIT
       cast picks the variant from the target type in the N_CAST arm.) */
    return float_to_int128_mem (c2m_ctx, scalar, get_mir_type (c2m_ctx, scalar_type), TRUE);
  res = int128_temp (c2m_ctx);
  assert (integer_type_p (scalar_type) && !vector_type_p (scalar_type));
  if (int128_type_p (scalar_type)) {
    assert (scalar.mir_op.mode == MIR_OP_MEM);
    block_move (c2m_ctx, res, scalar, 16);
    return res;
  }
  low = cast (c2m_ctx, scalar, MIR_T_U64, TRUE);
  if (signed_integer_type_p (scalar_type)) {
    op_t sign_src = cast (c2m_ctx, scalar, MIR_T_I64, TRUE);

    high = get_new_temp (c2m_ctx, MIR_T_I64);
    emit3 (c2m_ctx, MIR_RSH, high.mir_op, sign_src.mir_op, MIR_new_int_op (ctx, 63));
  } else {
    high = new_op (NULL, MIR_new_uint_op (ctx, 0));
  }
  store_int128_halves (c2m_ctx, res, low, high);
  return res;
}

static op_t materialize_int128_scalar_node (c2m_ctx_t c2m_ctx, node_t node,
                                            struct type *type) {
  struct expr *expr = node->attr;
  op_t op;

  assert (integer_type_p (type) || floating_type_p (type));
  if (int128_type_p (type)) {
    if (expr->const_p) return const_int128_mem (c2m_ctx, expr);
    op = gen (c2m_ctx, node, NULL, NULL, FALSE, NULL, NULL);
    assert (op.mir_op.mode == MIR_OP_MEM);
    return op;
  }
  op = val_gen (c2m_ctx, node);
  return scalar_to_int128_mem (c2m_ctx, op, type);
}

static op_t materialize_int128_vector_node (c2m_ctx_t c2m_ctx, node_t node,
                                            struct type *type) {
  if (int128_vector_type_p (c2m_ctx, type)) {
    op_t op = val_gen (c2m_ctx, node);

    return materialize_vector_operand (c2m_ctx, op, type);
  }
  return materialize_int128_scalar_node (c2m_ctx, node, type);
}

/* SCALAR __int128 ops reuse the one-lane int128 vector lowering: a one-lane
   int128 vector and a scalar int128 have the same 16-byte memory layout, and
   the vector emitters only consult the vector type for element signedness and
   16-byte temp allocation.  Synthesize the one-lane vector type over the
   scalar int128 type at the dispatch site. */
static struct type *int128_one_lane_vector_type (c2m_ctx_t c2m_ctx, struct type *type,
                                                 node_t pos_node) {
  assert (int128_type_p (type));
  return create_vector_type_with_nel (c2m_ctx, type, 16, 1, pos_node);
}

static op_t materialize_int128_vector_value (c2m_ctx_t c2m_ctx, op_t op,
                                             struct type *type,
                                             struct type *vector_type) {
  op_t res;

  assert (int128_vector_type_p (c2m_ctx, vector_type));
  if (int128_vector_type_p (c2m_ctx, type)) return materialize_vector_operand (c2m_ctx, op, type);
  if (int128_type_p (type)) {
    assert (op.mir_op.mode == MIR_OP_MEM);
    res = vector_temp (c2m_ctx, vector_type);
    block_move (c2m_ctx, res, op, 16);
    return res;
  }
  return scalar_to_int128_mem (c2m_ctx, op, type);
}

static op_t const_int128_vector (c2m_ctx_t c2m_ctx, struct type *vector_type, uint64_t low,
                                 uint64_t high) {
  MIR_context_t ctx = c2m_ctx->ctx;
  op_t res = vector_temp (c2m_ctx, vector_type);
  op_t low_op = new_op (NULL, MIR_new_uint_op (ctx, low));
  op_t high_op = new_op (NULL, MIR_new_uint_op (ctx, high));

  assert (int128_vector_type_p (c2m_ctx, vector_type));
  store_int128_halves (c2m_ctx, res, low_op, high_op);
  return res;
}

static void load_int128_halves (c2m_ctx_t c2m_ctx, op_t src, MIR_type_t high_type,
                                op_t *low, op_t *high) {
  *low = get_new_temp (c2m_ctx, MIR_T_U64);
  *high = get_new_temp (c2m_ctx, high_type);
  emit2 (c2m_ctx, MIR_MOV, low->mir_op, int128_half_op (c2m_ctx, src, MIR_T_U64, 0).mir_op);
  emit2 (c2m_ctx, MIR_MOV, high->mir_op, int128_half_op (c2m_ctx, src, high_type, 8).mir_op);
}

static void emit_u64_mul_128 (c2m_ctx_t c2m_ctx, op_t op1, op_t op2, op_t *low_res,
                              op_t *high_res) {
  MIR_context_t ctx = c2m_ctx->ctx;
  op_t mask32 = new_op (NULL, MIR_new_uint_op (ctx, UINT64_C (0xffffffff)));
  op_t a0 = get_new_temp (c2m_ctx, MIR_T_U64), a1 = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t b0 = get_new_temp (c2m_ctx, MIR_T_U64), b1 = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t p0 = get_new_temp (c2m_ctx, MIR_T_U64), p1 = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t p2 = get_new_temp (c2m_ctx, MIR_T_U64), p3 = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t mid = get_new_temp (c2m_ctx, MIR_T_U64), mid_low = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t mid_high = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t carry_mid = get_new_temp (c2m_ctx, MIR_T_U64), carry_low = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t carry_mid_high = get_new_temp (c2m_ctx, MIR_T_U64);

  emit3 (c2m_ctx, MIR_AND, a0.mir_op, op1.mir_op, mask32.mir_op);
  emit3 (c2m_ctx, MIR_URSH, a1.mir_op, op1.mir_op, MIR_new_int_op (ctx, 32));
  emit3 (c2m_ctx, MIR_AND, b0.mir_op, op2.mir_op, mask32.mir_op);
  emit3 (c2m_ctx, MIR_URSH, b1.mir_op, op2.mir_op, MIR_new_int_op (ctx, 32));
  emit3 (c2m_ctx, MIR_MUL, p0.mir_op, a0.mir_op, b0.mir_op);
  emit3 (c2m_ctx, MIR_MUL, p1.mir_op, a0.mir_op, b1.mir_op);
  emit3 (c2m_ctx, MIR_MUL, p2.mir_op, a1.mir_op, b0.mir_op);
  emit3 (c2m_ctx, MIR_MUL, p3.mir_op, a1.mir_op, b1.mir_op);
  emit3 (c2m_ctx, MIR_ADD, mid.mir_op, p1.mir_op, p2.mir_op);
  emit3 (c2m_ctx, MIR_ULT, carry_mid.mir_op, mid.mir_op, p1.mir_op);
  emit3 (c2m_ctx, MIR_LSH, mid_low.mir_op, mid.mir_op, MIR_new_int_op (ctx, 32));
  emit3 (c2m_ctx, MIR_URSH, mid_high.mir_op, mid.mir_op, MIR_new_int_op (ctx, 32));
  *low_res = get_new_temp (c2m_ctx, MIR_T_U64);
  *high_res = get_new_temp (c2m_ctx, MIR_T_U64);
  emit3 (c2m_ctx, MIR_ADD, low_res->mir_op, p0.mir_op, mid_low.mir_op);
  emit3 (c2m_ctx, MIR_ULT, carry_low.mir_op, low_res->mir_op, p0.mir_op);
  emit3 (c2m_ctx, MIR_LSH, carry_mid_high.mir_op, carry_mid.mir_op, MIR_new_int_op (ctx, 32));
  emit3 (c2m_ctx, MIR_ADD, high_res->mir_op, p3.mir_op, mid_high.mir_op);
  emit3 (c2m_ctx, MIR_ADD, high_res->mir_op, high_res->mir_op, carry_mid_high.mir_op);
...
/* int128 <-> floating conversion helper call plumbing.  Mirrors the divmod
   helpers: the libgcc-named helper's native __int128 argument/result ABI is
   exactly two integer eightbytes, spelled as two u64 args / two u64 results. */
static MIR_item_t get_int128_conv_helper_item (c2m_ctx_t c2m_ctx, int to_float_p, MIR_type_t ft,
                                               int signed_p, MIR_item_t *proto_res) {
  gen_ctx_t gen_ctx = c2m_ctx->gen_ctx;
  MIR_context_t ctx = c2m_ctx->ctx;
  MIR_module_t module = curr_func->module;
  int fi = ft == MIR_T_F ? 0 : ft == MIR_T_D ? 1 : 2;
  int ii = (to_float_p ? 0 : 6) + (signed_p ? 0 : 3) + fi;
  static const char *const names[12]
    = {"__floattisf", "__floattidf",   "__floattixf",   "__floatuntisf",
       "__floatuntidf", "__floatuntixf", "__fixsfti",     "__fixdfti",
       "__fixxfti",     "__fixunssfti",  "__fixunsdfti",  "__fixunsxfti"};
  MIR_item_t *proto_slot
    = to_float_p ? &int128_to_float_protos[fi] : &float_to_int128_protos[fi];

  assert (ft == MIR_T_F || ft == MIR_T_D || ft == MIR_T_LD);
  if (*proto_slot == NULL) {
    MIR_type_t res_types[2];
    MIR_var_t vars[2];
    char pname[32];
  ...
  if (int128_conv_items[ii] == NULL) {
    void *addr = MIR_int128_helper_resolver (names[ii]);

    int128_conv_items[ii] = MIR_new_import (ctx, names[ii]);
    if (addr != NULL) MIR_load_external (ctx, names[ii], addr);
    move_item_to_module_start (module, int128_conv_items[ii]);
  }
  *proto_res = *proto_slot;
  return int128_conv_items[ii];
}

/* Scalar __int128 memory value -> floating register value. */
static op_t int128_mem_to_float (c2m_ctx_t c2m_ctx, op_t mem, MIR_type_t ft, int signed_p) {
  MIR_context_t ctx = c2m_ctx->ctx;
  MIR_item_t proto;
  MIR_item_t item = get_int128_conv_helper_item (c2m_ctx, TRUE, ft, signed_p, &proto);
  op_t low, high, res = get_new_temp (c2m_ctx, ft);
  MIR_op_t args[5];

  load_int128_halves (c2m_ctx, mem, MIR_T_U64, &low, &high);
...
/* Floating register value -> scalar __int128 memory value. */
static op_t float_to_int128_mem (c2m_ctx_t c2m_ctx, op_t fval, MIR_type_t ft, int signed_p) {
  MIR_context_t ctx = c2m_ctx->ctx;
  MIR_item_t proto;
...
static op_t emit_int128_vector_bin_op (c2m_ctx_t c2m_ctx, node_code_t code, op_t op1,
  ...
  op_t src1 = materialize_int128_vector_value (c2m_ctx, op1, type1, vector_type);
  op_t src2 = materialize_int128_vector_value (c2m_ctx, op2, type2, vector_type);
  op_t low1, high1, low2, high2, low_res, high_res;

  load_int128_halves (c2m_ctx, src1, MIR_T_U64, &low1, &high1);
  load_int128_halves (c2m_ctx, src2, MIR_T_U64, &low2, &high2);
  low_res = get_new_temp (c2m_ctx, MIR_T_U64);
  high_res = get_new_temp (c2m_ctx, MIR_T_U64);
  switch (code) {
  case N_ADD:
  case N_ADD_ASSIGN: {
    op_t carry = get_new_temp (c2m_ctx, MIR_T_U64);

    emit3 (c2m_ctx, MIR_ADD, low_res.mir_op, low1.mir_op, low2.mir_op);
    emit3 (c2m_ctx, MIR_ULT, carry.mir_op, low_res.mir_op, low1.mir_op);
    emit3 (c2m_ctx, MIR_ADD, high_res.mir_op, high1.mir_op, high2.mir_op);
    emit3 (c2m_ctx, MIR_ADD, high_res.mir_op, high_res.mir_op, carry.mir_op);
    break;
  }
  case N_SUB:
  case N_SUB_ASSIGN: {
    op_t borrow = get_new_temp (c2m_ctx, MIR_T_U64);

    emit3 (c2m_ctx, MIR_ULT, borrow.mir_op, low1.mir_op, low2.mir_op);
    emit3 (c2m_ctx, MIR_SUB, low_res.mir_op, low1.mir_op, low2.mir_op);
    emit3 (c2m_ctx, MIR_SUB, high_res.mir_op, high1.mir_op, high2.mir_op);
    emit3 (c2m_ctx, MIR_SUB, high_res.mir_op, high_res.mir_op, borrow.mir_op);
    break;
  }
  case N_AND:
  case N_AND_ASSIGN:
    emit3 (c2m_ctx, MIR_AND, low_res.mir_op, low1.mir_op, low2.mir_op);
    emit3 (c2m_ctx, MIR_AND, high_res.mir_op, high1.mir_op, high2.mir_op);
    break;
  case N_OR:
  case N_OR_ASSIGN:
    emit3 (c2m_ctx, MIR_OR, low_res.mir_op, low1.mir_op, low2.mir_op);
    emit3 (c2m_ctx, MIR_OR, high_res.mir_op, high1.mir_op, high2.mir_op);
    break;
  case N_XOR:
  case N_XOR_ASSIGN:
    emit3 (c2m_ctx, MIR_XOR, low_res.mir_op, low1.mir_op, low2.mir_op);
    emit3 (c2m_ctx, MIR_XOR, high_res.mir_op, high1.mir_op, high2.mir_op);
    break;
  case N_MUL:
  case N_MUL_ASSIGN: {
    op_t low_prod, high_prod, cross1 = get_new_temp (c2m_ctx, MIR_T_U64);
    op_t cross2 = get_new_temp (c2m_ctx, MIR_T_U64);

    emit_u64_mul_128 (c2m_ctx, low1, low2, &low_prod, &high_prod);
    emit3 (c2m_ctx, MIR_MUL, cross1.mir_op, low1.mir_op, high2.mir_op);
    emit3 (c2m_ctx, MIR_MUL, cross2.mir_op, high1.mir_op, low2.mir_op);
    emit2 (c2m_ctx, MIR_MOV, low_res.mir_op, low_prod.mir_op);
    emit3 (c2m_ctx, MIR_ADD, high_res.mir_op, high_prod.mir_op, cross1.mir_op);
...
static op_t int128_vector_shift_count (c2m_ctx_t c2m_ctx, op_t op, struct type *type) {
  op_t src, count;

  if (int128_vector_type_p (c2m_ctx, type) || int128_type_p (type)) {
    src = int128_vector_type_p (c2m_ctx, type) ? materialize_vector_operand (c2m_ctx, op, type)
                                               : op;
    assert (src.mir_op.mode == MIR_OP_MEM);
    count = get_new_temp (c2m_ctx, MIR_T_U64);
    emit2 (c2m_ctx, MIR_MOV, count.mir_op, int128_half_op (c2m_ctx, src, MIR_T_U64, 0).mir_op);
    return count;
  }
  return cast (c2m_ctx, op, MIR_T_U64, TRUE);
}

static op_t emit_int128_vector_shift_op (c2m_ctx_t c2m_ctx, node_code_t code, op_t op1,
                                         struct type *type1, op_t op2, struct type *type2,
                                         struct type *vector_type, op_t dest) {
  MIR_context_t ctx = c2m_ctx->ctx;
  int signed_rshift = (code == N_RSH || code == N_RSH_ASSIGN)
                      && signed_integer_type_p (vector_type->u.vector_type->el_type);
  op_t src = materialize_int128_vector_value (c2m_ctx, op1, type1, vector_type);
  op_t count = int128_vector_shift_count (c2m_ctx, op2, type2);
  op_t low, high, low_res = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t high_res = get_new_temp (c2m_ctx, MIR_T_U64);
  MIR_label_t zero_label = MIR_new_label (ctx), lt64_label = MIR_new_label (ctx);
  MIR_label_t lt128_label = MIR_new_label (ctx), end_label = MIR_new_label (ctx);

  load_int128_halves (c2m_ctx, src, signed_rshift ? MIR_T_I64 : MIR_T_U64, &low, &high);
  emit3 (c2m_ctx, MIR_BEQ, MIR_new_label_op (ctx, zero_label), count.mir_op,
         MIR_new_int_op (ctx, 0));
  emit3 (c2m_ctx, MIR_UBLT, MIR_new_label_op (ctx, lt64_label), count.mir_op,
         MIR_new_int_op (ctx, 64));
  emit3 (c2m_ctx, MIR_UBLT, MIR_new_label_op (ctx, lt128_label), count.mir_op,
         MIR_new_int_op (ctx, 128));
  if (signed_rshift) {
    emit3 (c2m_ctx, MIR_RSH, high_res.mir_op, high.mir_op, MIR_new_int_op (ctx, 63));
    emit2 (c2m_ctx, MIR_MOV, low_res.mir_op, high_res.mir_op);
  } else {
    emit2 (c2m_ctx, MIR_MOV, low_res.mir_op, MIR_new_uint_op (ctx, 0));
    emit2 (c2m_ctx, MIR_MOV, high_res.mir_op, MIR_new_uint_op (ctx, 0));
  }
  emit1 (c2m_ctx, MIR_JMP, MIR_new_label_op (ctx, end_label));

  emit_label_insn_opt (c2m_ctx, zero_label);
  emit2 (c2m_ctx, MIR_MOV, low_res.mir_op, low.mir_op);
  emit2 (c2m_ctx, MIR_MOV, high_res.mir_op, high.mir_op);
  emit1 (c2m_ctx, MIR_JMP, MIR_new_label_op (ctx, end_label));

  emit_label_insn_opt (c2m_ctx, lt64_label);
  if (code == N_LSH || code == N_LSH_ASSIGN) {
    op_t inv_count = get_new_temp (c2m_ctx, MIR_T_U64);
    op_t carry = get_new_temp (c2m_ctx, MIR_T_U64);
    op_t high_shifted = get_new_temp (c2m_ctx, MIR_T_U64);

    emit3 (c2m_ctx, MIR_SUB, inv_count.mir_op, MIR_new_int_op (ctx, 64), count.mir_op);
    emit3 (c2m_ctx, MIR_URSH, carry.mir_op, low.mir_op, inv_count.mir_op);
    emit3 (c2m_ctx, MIR_LSH, low_res.mir_op, low.mir_op, count.mir_op);
    emit3 (c2m_ctx, MIR_LSH, high_shifted.mir_op, high.mir_op, count.mir_op);
    emit3 (c2m_ctx, MIR_OR, high_res.mir_op, high_shifted.mir_op, carry.mir_op);
  } else {
    op_t inv_count = get_new_temp (c2m_ctx, MIR_T_U64);
    op_t carry = get_new_temp (c2m_ctx, MIR_T_U64);
    op_t low_shifted = get_new_temp (c2m_ctx, MIR_T_U64);

    emit3 (c2m_ctx, MIR_SUB, inv_count.mir_op, MIR_new_int_op (ctx, 64), count.mir_op);
    emit3 (c2m_ctx, MIR_LSH, carry.mir_op, high.mir_op, inv_count.mir_op);
    emit3 (c2m_ctx, MIR_URSH, low_shifted.mir_op, low.mir_op, count.mir_op);
    emit3 (c2m_ctx, MIR_OR, low_res.mir_op, low_shifted.mir_op, carry.mir_op);
    emit3 (c2m_ctx, signed_rshift ? MIR_RSH : MIR_URSH, high_res.mir_op, high.mir_op,
           count.mir_op);
  }
  emit1 (c2m_ctx, MIR_JMP, MIR_new_label_op (ctx, end_label));

  emit_label_insn_opt (c2m_ctx, lt128_label);
  {
    op_t count_minus_64 = get_new_temp (c2m_ctx, MIR_T_U64);

    emit3 (c2m_ctx, MIR_SUB, count_minus_64.mir_op, count.mir_op, MIR_new_int_op (ctx, 64));
    if (code == N_LSH || code == N_LSH_ASSIGN) {
      emit2 (c2m_ctx, MIR_MOV, low_res.mir_op, MIR_new_uint_op (ctx, 0));
      emit3 (c2m_ctx, MIR_LSH, high_res.mir_op, low.mir_op, count_minus_64.mir_op);
    } else if (signed_rshift) {
      emit3 (c2m_ctx, MIR_RSH, low_res.mir_op, high.mir_op, count_minus_64.mir_op);
      emit3 (c2m_ctx, MIR_RSH, high_res.mir_op, high.mir_op, MIR_new_int_op (ctx, 63));
    } else {
      emit3 (c2m_ctx, MIR_URSH, low_res.mir_op, high.mir_op, count_minus_64.mir_op);
      emit2 (c2m_ctx, MIR_MOV, high_res.mir_op, MIR_new_uint_op (ctx, 0));
    }
  }
...
static op_t const_v128_i32_shift_count (c2m_ctx_t c2m_ctx, struct type *vector_type,
                                        uint64_t val) {
  MIR_context_t ctx = c2m_ctx->ctx;
  op_t res = const_integer_vector (c2m_ctx, vector_type, 0);
  op_t count = new_op (NULL, MIR_new_uint_op (ctx, val));
  op_t low_count = res;
...
static op_t emit_v128_i32_packed_const_shift_op (c2m_ctx_t c2m_ctx, MIR_insn_code_t code,
                                                 op_t op1, struct type *type1,
                                                 struct type *vector_type, uint64_t count,
                                                 op_t dest) {
  op_t reg = get_new_temp (c2m_ctx, MIR_T_V128);
  op_t count_op;

  op1 = prepare_v128_i32_operand (c2m_ctx, op1, type1, vector_type);
  count_op = const_v128_i32_shift_count (c2m_ctx, vector_type, count);
  count_op = force_v128_reg (c2m_ctx, count_op);
  emit3 (c2m_ctx, code, reg.mir_op, op1.mir_op, count_op.mir_op);
  store_v128 (c2m_ctx, dest, reg);
  return dest;
}
...
static op_t emit_v128_i8_mul_op (c2m_ctx_t c2m_ctx, op_t op1, struct type *type1, op_t op2,
  ...
  op_t src1 = materialize_integer_vector_operand (c2m_ctx, op1, type1, vector_type);
  op_t src2 = materialize_integer_vector_operand (c2m_ctx, op2, type2, vector_type);
  op_t low1 = vector_temp (c2m_ctx, vector_type);
  op_t low2 = vector_temp (c2m_ctx, vector_type);
  op_t high1_masked = vector_temp (c2m_ctx, vector_type);
  op_t high2_masked = vector_temp (c2m_ctx, vector_type);
  op_t high1 = vector_temp (c2m_ctx, vector_type);
  op_t high2 = vector_temp (c2m_ctx, vector_type);
  op_t low_prod = vector_temp (c2m_ctx, vector_type);
  op_t high_prod = vector_temp (c2m_ctx, vector_type);
  op_t low_res = vector_temp (c2m_ctx, vector_type);
  op_t high_shifted = vector_temp (c2m_ctx, vector_type);
  op_t high_res = vector_temp (c2m_ctx, vector_type);

  assert (v128_i8_packed_mul_vector_type_p (c2m_ctx, vector_type));
  emit_v128_i32_bin_op (c2m_ctx, MIR_VAND, src1, vector_type, low_mask, vector_type, vector_type,
                        low1);
  emit_v128_i32_bin_op (c2m_ctx, MIR_VAND, src2, vector_type, low_mask, vector_type, vector_type,
                        low2);
  emit_v128_i32_bin_op (c2m_ctx, MIR_VAND, src1, vector_type, high_mask, vector_type,
                        vector_type, high1_masked);
  emit_v128_i32_bin_op (c2m_ctx, MIR_VAND, src2, vector_type, high_mask, vector_type,
                        vector_type, high2_masked);
  emit_v128_i32_packed_const_shift_op (c2m_ctx, MIR_VURSHI16, high1_masked, vector_type,
                                       vector_type, 8, high1);
  emit_v128_i32_packed_const_shift_op (c2m_ctx, MIR_VURSHI16, high2_masked, vector_type,
                                       vector_type, 8, high2);
  emit_v128_i32_bin_op (c2m_ctx, MIR_VMULI16, low1, vector_type, low2, vector_type, vector_type,
                        low_prod);
  emit_v128_i32_bin_op (c2m_ctx, MIR_VMULI16, high1, vector_type, high2, vector_type,
                        vector_type, high_prod);
  emit_v128_i32_bin_op (c2m_ctx, MIR_VAND, low_prod, vector_type, low_mask, vector_type,
                        vector_type, low_res);
  emit_v128_i32_packed_const_shift_op (c2m_ctx, MIR_VLSHI16, high_prod, vector_type,
                                       vector_type, 8, high_shifted);
  emit_v128_i32_bin_op (c2m_ctx, MIR_VAND, high_shifted, vector_type, high_mask, vector_type,
                        vector_type, high_res);
  return emit_v128_i32_bin_op (c2m_ctx, MIR_VOR, low_res, vector_type, high_res, vector_type,
                               vector_type, dest);
}
...
static op_t emit_v128_i32_gt_op (c2m_ctx_t c2m_ctx, op_t op1, struct type *type1, op_t op2,
                                 struct type *type2, struct type *vector_type, op_t dest) {
  MIR_insn_code_t gt_code = get_v128_i32_packed_gt_insn_code (c2m_ctx, vector_type);
  ...
  emit_v128_i32_bin_op (c2m_ctx, MIR_VXOR, op2, type2, sign_bit, vector_type, vector_type,
                        biased_op2);
  return emit_v128_i32_bin_op (c2m_ctx, gt_code, biased_op1, vector_type, biased_op2, vector_type,
                               vector_type, dest);
}

static op_t emit_v128_i64_signed_scalar_rshift_op (c2m_ctx_t c2m_ctx, op_t op1,
                                                   struct type *type1, op_t op2,
                                                   struct type *type2,
                                                   struct type *vector_type, op_t dest) {
  op_t logical = vector_temp (c2m_ctx, vector_type);
  op_t sign_bit = const_unsigned_integer_vector (c2m_ctx, vector_type, UINT64_C (1) << 63);
  op_t shifted_sign_bit = vector_temp (c2m_ctx, vector_type);
  op_t biased = vector_temp (c2m_ctx, vector_type);

  assert (v128_i64_signed_scalar_rshift_op_p (c2m_ctx, N_RSH, vector_type, type2));
  emit_v128_i32_packed_scalar_shift_op (c2m_ctx, MIR_VURSHI64, op1, type1, op2, type2,
                                        vector_type, logical);
  emit_v128_i32_packed_scalar_shift_op (c2m_ctx, MIR_VURSHI64, sign_bit, vector_type, op2, type2,
                                        vector_type, shifted_sign_bit);
...
static op_t emit_v128_i32_lane_cmp_op (c2m_ctx_t c2m_ctx, node_code_t code, op_t op1,
  ...
  op2 = materialize_integer_vector_operand (c2m_ctx, op2, type2, vector_type);
...
static op_t gen_int128_vector_cmp_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  MIR_context_t ctx = c2m_ctx->ctx;
  node_t node1 = NL_HEAD (r->u.ops), node2 = NL_EL (r->u.ops, 1);
  struct type *type = ((struct expr *) r->attr)->type;
  struct type *type1 = ((struct expr *) node1->attr)->type;
  struct type *type2 = ((struct expr *) node2->attr)->type;
  ...
  op_t high2 = get_new_temp (c2m_ctx, high_type);
  op_t cmp = get_new_temp (c2m_ctx, MIR_T_I64);
  op_t mask = get_new_temp (c2m_ctx, MIR_T_I64);

  assert (int128_vector_type_p (c2m_ctx, type));
  emit2 (c2m_ctx, MIR_MOV, low1.mir_op, int128_half_op (c2m_ctx, src1, MIR_T_U64, 0).mir_op);
  emit2 (c2m_ctx, MIR_MOV, high1.mir_op, int128_half_op (c2m_ctx, src1, high_type, 8).mir_op);
  emit2 (c2m_ctx, MIR_MOV, low2.mir_op, int128_half_op (c2m_ctx, src2, MIR_T_U64, 0).mir_op);
  emit2 (c2m_ctx, MIR_MOV, high2.mir_op, int128_half_op (c2m_ctx, src2, high_type, 8).mir_op);
  if (r->code == N_EQ || r->code == N_NE) {
    op_t cmp_low = get_new_temp (c2m_ctx, MIR_T_I64);
    op_t cmp_high = get_new_temp (c2m_ctx, MIR_T_I64);

    emit3 (c2m_ctx, r->code == N_EQ ? MIR_EQ : MIR_NE, cmp_low.mir_op, low1.mir_op,
           low2.mir_op);
    emit3 (c2m_ctx, r->code == N_EQ ? MIR_EQ : MIR_NE, cmp_high.mir_op, high1.mir_op,
           high2.mir_op);
    emit3 (c2m_ctx, r->code == N_EQ ? MIR_AND : MIR_OR, cmp.mir_op, cmp_low.mir_op,
           cmp_high.mir_op);
  } else {
    op_t cmp_high = get_new_temp (c2m_ctx, MIR_T_I64);
    ...
    MIR_insn_code_t low_cmp = (r->code == N_LT   ? MIR_ULT
                               : r->code == N_LE ? MIR_ULE
                               : r->code == N_GT ? MIR_UGT
                                                 : MIR_UGE);

    emit3 (c2m_ctx, high_cmp, cmp_high.mir_op, high1.mir_op, high2.mir_op);
    emit3 (c2m_ctx, MIR_EQ, eq_high.mir_op, high1.mir_op, high2.mir_op);
    emit3 (c2m_ctx, low_cmp, cmp_low.mir_op, low1.mir_op, low2.mir_op);
    emit3 (c2m_ctx, MIR_AND, low_if_eq.mir_op, eq_high.mir_op, cmp_low.mir_op);
    emit3 (c2m_ctx, MIR_OR, cmp.mir_op, cmp_high.mir_op, low_if_eq.mir_op);
  }
  emit3 (c2m_ctx, MIR_SUB, mask.mir_op, MIR_new_int_op (ctx, 0), cmp.mir_op);
  store_int128_halves (c2m_ctx, dest, mask, mask);
  return dest;
}

static op_t gen_int128_vector_unary_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  struct type *type = ((struct expr *) r->attr)->type;
  op_t op = val_gen (c2m_ctx, NL_HEAD (r->u.ops));
  op_t src = materialize_int128_vector_value (c2m_ctx, op, type, type);
  op_t dest = desirable_dest != NULL ? *desirable_dest : vector_temp (c2m_ctx, type);

  assert (int128_vector_type_p (c2m_ctx, type));
  if (r->code == N_ADD) {
    block_move (c2m_ctx, dest, src, raw_type_size (c2m_ctx, type));
    return dest;
  } else if (r->code == N_SUB) {
    op_t zero = const_int128_vector (c2m_ctx, type, 0, 0);

    return emit_int128_vector_bin_op (c2m_ctx, N_SUB, zero, type, src, type, type, dest);
  } else {
    op_t all_ones = const_int128_vector (c2m_ctx, type, UINT64_MAX, UINT64_MAX);

    assert (r->code == N_BITWISE_NOT);
    return emit_int128_vector_bin_op (c2m_ctx, N_XOR, src, type, all_ones, type, type, dest);
  }
}

static op_t gen_int128_vector_bin_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  struct type *type = ((struct expr *) r->attr)->type;
  ...
  return emit_int128_vector_bin_op (c2m_ctx, r->code, op1, type1, op2, type2, type, dest);
}

static op_t gen_int128_vector_shift_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  struct type *type = ((struct expr *) r->attr)->type;
  struct type *type1 = ((struct expr *) NL_HEAD (r->u.ops)->attr)->type;
  struct type *type2 = ((struct expr *) NL_EL (r->u.ops, 1)->attr)->type;
  op_t op1 = val_gen (c2m_ctx, NL_HEAD (r->u.ops));
  ...
  return emit_int128_vector_shift_op (c2m_ctx, r->code, op1, type1, op2, type2, type, dest);
}

/* Scalar __int128 expression ops.  Operands are materialized at the NODE
   level (const-correct: a >64-bit constant operand goes through
   const_int128_mem, never through a truncating val_gen), then handed to the
   one-lane-vector emitters with the operand type rewritten to the already
   materialized int128 result type. */
static op_t gen_int128_scalar_bin_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  struct type *type = ((struct expr *) r->attr)->type;
  node_t node1 = NL_HEAD (r->u.ops), node2 = NL_EL (r->u.ops, 1);
  struct type *type1 = ((struct expr *) node1->attr)->type;
  struct type *type2 = ((struct expr *) node2->attr)->type;
  struct type *vtype = int128_one_lane_vector_type (c2m_ctx, type, r);
  op_t op1 = materialize_int128_scalar_node (c2m_ctx, node1, type1);
  op_t op2 = materialize_int128_scalar_node (c2m_ctx, node2, type2);
  op_t dest = desirable_dest != NULL ? *desirable_dest : int128_temp (c2m_ctx);

  return emit_int128_vector_bin_op (c2m_ctx, r->code, op1, type, op2, type, vtype, dest);
}

static op_t gen_int128_scalar_shift_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  struct type *type = ((struct expr *) r->attr)->type;
  node_t node1 = NL_HEAD (r->u.ops), node2 = NL_EL (r->u.ops, 1);
  struct type *type1 = ((struct expr *) node1->attr)->type;
  struct type *type2 = ((struct expr *) node2->attr)->type;
  struct type *vtype = int128_one_lane_vector_type (c2m_ctx, type, r);
  op_t op1 = materialize_int128_scalar_node (c2m_ctx, node1, type1);
  op_t op2 = int128_type_p (type2) ? materialize_int128_scalar_node (c2m_ctx, node2, type2)
                                   : val_gen (c2m_ctx, node2);
  op_t dest = desirable_dest != NULL ? *desirable_dest : int128_temp (c2m_ctx);

  return emit_int128_vector_shift_op (c2m_ctx, r->code, op1, type, op2, type2, vtype, dest);
}

static op_t gen_int128_scalar_unary_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  struct type *type = ((struct expr *) r->attr)->type;
  node_t node1 = NL_HEAD (r->u.ops);
  struct type *type1 = ((struct expr *) node1->attr)->type;
  struct type *vtype = int128_one_lane_vector_type (c2m_ctx, type, r);
  op_t src = materialize_int128_scalar_node (c2m_ctx, node1, type1);
  op_t dest = desirable_dest != NULL ? *desirable_dest : int128_temp (c2m_ctx);

  if (r->code == N_ADD) {
    block_move (c2m_ctx, dest, src, 16);
    return dest;
  } else if (r->code == N_SUB) {
    op_t zero = const_int128_vector (c2m_ctx, vtype, 0, 0);

    return emit_int128_vector_bin_op (c2m_ctx, N_SUB, zero, type, src, type, vtype, dest);
  } else {
    op_t all_ones = const_int128_vector (c2m_ctx, vtype, UINT64_MAX, UINT64_MAX);

    assert (r->code == N_BITWISE_NOT);
    return emit_int128_vector_bin_op (c2m_ctx, N_XOR, src, type, all_ones, type, vtype, dest);
  }
}

/* Narrow a scalar __int128 memory value to a 64-bit-or-narrower scalar type:
   the low half, then the ordinary scalar cast.  A floating target converts
   the full 128-bit value through the libgcc-shaped helper; signed_p is the
   SOURCE int128 signedness. */
static op_t int128_mem_to_scalar (c2m_ctx_t c2m_ctx, op_t mem, MIR_type_t t, int signed_p) {
  op_t low;

  if (t == MIR_T_F || t == MIR_T_D || t == MIR_T_LD)
    return int128_mem_to_float (c2m_ctx, mem, t, signed_p);
  low = get_new_temp (c2m_ctx, MIR_T_I64);
  assert (mem.mir_op.mode == MIR_OP_MEM);
  emit2 (c2m_ctx, MIR_MOV, low.mir_op, int128_half_op (c2m_ctx, mem, MIR_T_I64, 0).mir_op);
  return cast (c2m_ctx, low, t, TRUE);
}

/* Branch to TARGET when the 128-bit value in (low, high) regs is < (or <=
   when le_p) the 128-bit constant (kl, kh); signedness applies to the high
   word, the low word always compares unsigned. */
static void emit_int128_branch_lt_le (c2m_ctx_t c2m_ctx, MIR_label_t target, int le_p,
                                      int signed_p, op_t low, op_t high, mir_ullong kl,
                                      mir_ullong kh) {
  MIR_context_t ctx = c2m_ctx->ctx;
  MIR_label_t skip = MIR_new_label (ctx);
  MIR_op_t kh_op
    = signed_p ? MIR_new_int_op (ctx, (mir_llong) kh) : MIR_new_uint_op (ctx, kh);

  emit3 (c2m_ctx, signed_p ? MIR_BLT : MIR_UBLT, MIR_new_label_op (ctx, target), high.mir_op,
         kh_op);
  emit3 (c2m_ctx, MIR_BNE, MIR_new_label_op (ctx, skip), high.mir_op, kh_op);
...
static op_t int128_scalar_truth_val (c2m_ctx_t c2m_ctx, op_t src) {
  MIR_context_t ctx = c2m_ctx->ctx;
  op_t low = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t high = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t res = get_new_temp (c2m_ctx, MIR_T_I64);

  assert (src.mir_op.mode == MIR_OP_MEM);
  emit2 (c2m_ctx, MIR_MOV, low.mir_op, int128_half_op (c2m_ctx, src, MIR_T_U64, 0).mir_op);
  emit2 (c2m_ctx, MIR_MOV, high.mir_op, int128_half_op (c2m_ctx, src, MIR_T_U64, 8).mir_op);
  emit3 (c2m_ctx, MIR_OR, res.mir_op, low.mir_op, high.mir_op);
  emit3 (c2m_ctx, MIR_NE, res.mir_op, res.mir_op, MIR_new_int_op (ctx, 0));
  return res;
}

static op_t gen_int128_scalar_cmp_op (c2m_ctx_t c2m_ctx, node_t r, struct type *cmp_type,
                                      MIR_label_t true_label, MIR_label_t false_label) {
  MIR_context_t ctx = c2m_ctx->ctx;
  node_t node1 = NL_HEAD (r->u.ops), node2 = NL_EL (r->u.ops, 1);
  struct type *type1 = ((struct expr *) node1->attr)->type;
  struct type *type2 = ((struct expr *) node2->attr)->type;
  MIR_type_t high_type = signed_integer_type_p (cmp_type) ? MIR_T_I64 : MIR_T_U64;
  op_t src1 = materialize_int128_scalar_node (c2m_ctx, node1, type1);
  op_t src2 = materialize_int128_scalar_node (c2m_ctx, node2, type2);
  op_t low1 = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t high1 = get_new_temp (c2m_ctx, high_type);
  op_t low2 = get_new_temp (c2m_ctx, MIR_T_U64);
  op_t high2 = get_new_temp (c2m_ctx, high_type);
  ...
  if (r->code == N_EQ || r->code == N_NE) {
    ...
    emit3 (c2m_ctx, r->code == N_EQ ? MIR_EQ : MIR_NE, cmp_high.mir_op, high1.mir_op,
           high2.mir_op);
    emit3 (c2m_ctx, r->code == N_EQ ? MIR_AND : MIR_OR, res.mir_op, cmp_low.mir_op,
           cmp_high.mir_op);
  } else {
    op_t cmp_high = get_new_temp (c2m_ctx, MIR_T_I64);
    op_t eq_high = get_new_temp (c2m_ctx, MIR_T_I64);
    op_t cmp_low = get_new_temp (c2m_ctx, MIR_T_I64);
    op_t low_if_eq = get_new_temp (c2m_ctx, MIR_T_I64);
    int signed_p = signed_integer_type_p (cmp_type);
    MIR_insn_code_t high_cmp
      = (r->code == N_LT || r->code == N_LE ? (signed_p ? MIR_LT : MIR_ULT)
                                             : (signed_p ? MIR_GT : MIR_UGT));
    MIR_insn_code_t low_cmp = (r->code == N_LT   ? MIR_ULT
    ...
    emit3 (c2m_ctx, low_cmp, cmp_low.mir_op, low1.mir_op, low2.mir_op);
    emit3 (c2m_ctx, MIR_AND, low_if_eq.mir_op, eq_high.mir_op, cmp_low.mir_op);
    emit3 (c2m_ctx, MIR_OR, res.mir_op, cmp_high.mir_op, low_if_eq.mir_op);
  }
  if (true_label != NULL) {
    emit3 (c2m_ctx, MIR_BNE, MIR_new_label_op (ctx, true_label), res.mir_op,
           MIR_new_int_op (ctx, 0));
...
static op_t emit_v128_i32_arith_op (c2m_ctx_t c2m_ctx, node_code_t code, op_t op1,
                                    struct type *type1, op_t op2, struct type *type2,
                                    struct type *vector_type, op_t dest);
static op_t emit_v128_i32_lane_bin_op (c2m_ctx_t c2m_ctx, MIR_insn_code_t insn_code, op_t op1,
                                       struct type *type1, op_t op2, struct type *type2,
                                       struct type *vector_type, op_t dest);
...
static op_t gen_v128_i32_unary_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  ...
  if (r->code == N_ADD) {
    op = materialize_integer_vector_operand (c2m_ctx, op, type, type);
    block_move (c2m_ctx, dest, op, raw_type_size (c2m_ctx, type));
    return dest;
  } else if (r->code == N_SUB) {
    op_t zero = const_integer_vector (c2m_ctx, type, 0);

    if (v128_i32_packed_add_sub_vector_type_p (c2m_ctx, type))
      return emit_v128_i32_bin_op (c2m_ctx,
                                   get_v128_i32_packed_add_sub_insn_code (c2m_ctx, N_SUB, type),
                                   zero, type, op, type, type, dest);
    return emit_v128_i32_arith_op (c2m_ctx, N_SUB, zero, type, op, type, type, dest);
  } else {
    op_t all_ones = const_integer_vector (c2m_ctx, type, -1);

    assert (r->code == N_BITWISE_NOT);
    if (v128_i32_vector_type_p (c2m_ctx, type))
      return emit_v128_i32_bin_op (c2m_ctx, MIR_VXOR, op, type, all_ones, type, type, dest);
    return emit_v128_i32_lane_bin_op (c2m_ctx,
                                      get_integer_vector_bitwise_insn_code (c2m_ctx, N_XOR,
                                                                           type),
                                      op, type, all_ones, type, type, dest);
  }
}
...
static op_t gen_v128_i32_bin_op (c2m_ctx_t c2m_ctx, node_t r, MIR_insn_code_t code,
  ...
  if (r->code == N_ADD || r->code == N_SUB) {
    if (v128_i32_packed_add_sub_vector_type_p (c2m_ctx, type)) {
      code = get_v128_i32_packed_add_sub_insn_code (c2m_ctx, r->code, type);
    } else {
  ...
  if (!v128_i32_vector_type_p (c2m_ctx, type))
    return emit_v128_i32_lane_bin_op (c2m_ctx,
                                      get_integer_vector_bitwise_insn_code (c2m_ctx, r->code,
                                                                           type),
                                      op1, type1, op2, type2, type, dest);
...
static MIR_insn_code_t get_v128_i32_packed_scalar_shift_insn_code (c2m_ctx_t c2m_ctx,
  ...
  assert (lane_size == 2 || lane_size == 4 || lane_size == 8);
  if (code == N_LSH || code == N_LSH_ASSIGN)
    return lane_size == 2 ? MIR_VLSHI16 : lane_size == 4 ? MIR_VLSHI32 : MIR_VLSHI64;
  assert (code == N_RSH || code == N_RSH_ASSIGN);
  if (signed_p) {
    assert (lane_size == 2 || lane_size == 4);
    return lane_size == 2 ? MIR_VRSHI16 : MIR_VRSHI32;
  }
...
static op_t emit_v128_i32_shift_op (c2m_ctx_t c2m_ctx, node_code_t code, op_t op1,
                                    struct type *type1, op_t op2, struct type *type2,
                                    struct type *vector_type, op_t dest) {
  ...
  if (v128_i64_signed_scalar_rshift_op_p (c2m_ctx, code, vector_type, type2))
    return emit_v128_i64_signed_scalar_rshift_op (c2m_ctx, op1, type1, op2, type2, vector_type,
                                                  dest);
  if (v128_i32_packed_scalar_shift_op_p (c2m_ctx, code, vector_type, type2)) {
    MIR_insn_code_t packed_code
...
static op_t gen_v128_i32_shift_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  op_t op1 = val_gen (c2m_ctx, NL_HEAD (r->u.ops));
  op_t op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
  struct type *type = ((struct expr *) r->attr)->type;
  struct type *type1 = ((struct expr *) NL_HEAD (r->u.ops)->attr)->type;
  ...
  return emit_v128_i32_shift_op (c2m_ctx, r->code, op1, type1, op2, type2, type, dest);
}
...
static op_t emit_v128_i32_arith_op (c2m_ctx_t c2m_ctx, node_code_t code, op_t op1,
  ...
  if ((code == N_MUL || code == N_MUL_ASSIGN)
      && v128_i8_packed_mul_vector_type_p (c2m_ctx, vector_type))
    return emit_v128_i8_mul_op (c2m_ctx, op1, type1, op2, type2, vector_type, dest);
  if ((code == N_MUL || code == N_MUL_ASSIGN)
...
static op_t gen_v128_i32_arith_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  op_t op1 = val_gen (c2m_ctx, NL_HEAD (r->u.ops));
  op_t op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
  struct type *type = ((struct expr *) r->attr)->type;
  struct type *type1 = ((struct expr *) NL_HEAD (r->u.ops)->attr)->type;
...
static op_t materialize_v128_float_operand (c2m_ctx_t c2m_ctx, op_t op, struct type *op_type,
                                            struct type *vector_type) {
  if (!v128_float_vector_type_p (c2m_ctx, op_type))
...
static MIR_insn_code_t get_v128_float_arith_insn_code (c2m_ctx_t c2m_ctx, node_code_t code,
  ...
  assert (lane_type == MIR_T_F || lane_type == MIR_T_D);
  switch (code) {
  case N_ADD:
  case N_ADD_ASSIGN: return lane_type == MIR_T_F ? MIR_FADD : MIR_DADD;
  case N_SUB:
  case N_SUB_ASSIGN: return lane_type == MIR_T_F ? MIR_FSUB : MIR_DSUB;
  case N_MUL:
  case N_MUL_ASSIGN: return lane_type == MIR_T_F ? MIR_FMUL : MIR_DMUL;
  case N_DIV:
  case N_DIV_ASSIGN: return lane_type == MIR_T_F ? MIR_FDIV : MIR_DDIV;
  default: assert (FALSE); return MIR_INSN_BOUND;
  }
}
...
static MIR_insn_code_t get_v128_float_cmp_insn_code (c2m_ctx_t c2m_ctx, node_code_t code,
  ...
  assert (lane_type == MIR_T_F || lane_type == MIR_T_D);
  switch (code) {
  case N_EQ: return lane_type == MIR_T_F ? MIR_FEQ : MIR_DEQ;
  case N_NE: return lane_type == MIR_T_F ? MIR_FNE : MIR_DNE;
  case N_LT: return lane_type == MIR_T_F ? MIR_FLT : MIR_DLT;
  case N_LE: return lane_type == MIR_T_F ? MIR_FLE : MIR_DLE;
  case N_GT: return lane_type == MIR_T_F ? MIR_FGT : MIR_DGT;
  case N_GE: return lane_type == MIR_T_F ? MIR_FGE : MIR_DGE;
  default: assert (FALSE); return MIR_INSN_BOUND;
  }
}

static MIR_insn_code_t get_v128_f32_cmp_insn_code (node_code_t code, int *swap_operands_p) {
  *swap_operands_p = FALSE;
  switch (code) {
  case N_EQ: return MIR_VEQF32;
  case N_NE: return MIR_VNEF32;
  case N_LT: return MIR_VLTF32;
  case N_LE: return MIR_VLEF32;
  case N_GT:
...
static op_t emit_v128_float_arith_op (c2m_ctx_t c2m_ctx, node_code_t code, op_t op1,
  ...
  for (mir_size_t offset = 0; offset < raw_type_size (c2m_ctx, vector_type);
       offset += lane_size) {
    op_t lane1 = vector_lane_op (op1, lane_type, offset);
    op_t lane2 = vector_lane_op (op2, lane_type, offset);
    op_t dest_lane = vector_lane_op (dest, lane_type, offset);
    op_t lane_res = get_new_temp (c2m_ctx, lane_type);
    op_t lane_val2 = get_new_temp (c2m_ctx, lane_type);

    emit2 (c2m_ctx, tp_mov (lane_type), lane_res.mir_op, lane1.mir_op);
    emit2 (c2m_ctx, tp_mov (lane_type), lane_val2.mir_op, lane2.mir_op);
    emit3 (c2m_ctx, insn_code, lane_res.mir_op, lane_res.mir_op, lane_val2.mir_op);
    emit_scalar_assign (c2m_ctx, dest_lane, &lane_res, lane_type, FALSE);
  }
  return dest;
}

static op_t gen_v128_float_arith_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  op_t op1 = val_gen (c2m_ctx, NL_HEAD (r->u.ops));
  op_t op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
  struct type *type = ((struct expr *) r->attr)->type;
  struct type *type1 = ((struct expr *) NL_HEAD (r->u.ops)->attr)->type;
...
static op_t gen_vector_inc_dec_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest,
  ...
  if (preserve_postfix_p) {
    if (int128_vector_type_p (c2m_ctx, type)) {
      old_val = materialize_vector_operand (c2m_ctx, old_val, type);
    } else if (supported_integer_vector_type_p (c2m_ctx, type)) {
  ...
  } else if (int128_vector_type_p (c2m_ctx, type)) {
    op_t one = const_int128_vector (c2m_ctx, type, 1, 0);

    emit_int128_vector_bin_op (c2m_ctx, arith_code, old_val, type, one, type, type, var);
  } else {
...
static op_t gen_v128_float_unary_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  ...
  assert (r->code == N_ADD || r->code == N_SUB);
  assert (lane_type == MIR_T_F || lane_type == MIR_T_D);
  for (mir_size_t offset = 0; offset < raw_type_size (c2m_ctx, type); offset += lane_size) {
    op_t src_lane = vector_lane_op (src, lane_type, offset);
    op_t dest_lane = vector_lane_op (dest, lane_type, offset);
    op_t lane_val = get_new_temp (c2m_ctx, lane_type);

    emit2 (c2m_ctx, tp_mov (lane_type), lane_val.mir_op, src_lane.mir_op);
    if (r->code == N_SUB) emit2 (c2m_ctx, neg_code, lane_val.mir_op, lane_val.mir_op);
    emit_scalar_assign (c2m_ctx, dest_lane, &lane_val, lane_type, FALSE);
  }
  return dest;
}

static op_t emit_v128_float_cmp_op (c2m_ctx_t c2m_ctx, node_code_t code, op_t op1,
                                    struct type *type1, op_t op2, struct type *type2,
                                    struct type *vector_type, struct type *mask_type,
                                    op_t dest) {
  MIR_context_t ctx = c2m_ctx->ctx;
  MIR_type_t lane_type = get_mir_type (c2m_ctx, vector_type->u.vector_type->el_type);
  MIR_type_t mask_lane_type = get_mir_type (c2m_ctx, mask_type->u.vector_type->el_type);
  mir_size_t lane_size = vector_lane_size (c2m_ctx, vector_type);
  MIR_insn_code_t insn_code = get_v128_float_cmp_insn_code (c2m_ctx, code, vector_type);

  if (v128_f32_packed_vector_type_p (c2m_ctx, vector_type)
      && v128_i32_packed_vector_type_p (c2m_ctx, mask_type)) {
    int swap_operands_p;
    op_t reg = get_new_temp (c2m_ctx, MIR_T_V128);
  ...
  for (mir_size_t offset = 0; offset < raw_type_size (c2m_ctx, vector_type);
    ...
    op_t lane1 = vector_lane_op (op1, lane_type, offset);
    op_t lane2 = vector_lane_op (op2, lane_type, offset);
    op_t dest_lane = vector_lane_op (dest, mask_lane_type, offset);
    op_t lane_val1 = get_new_temp (c2m_ctx, lane_type);
    op_t lane_val2 = get_new_temp (c2m_ctx, lane_type);
    op_t cmp = get_new_temp (c2m_ctx, MIR_T_I32);
    op_t mask_val;

    emit2 (c2m_ctx, tp_mov (lane_type), lane_val1.mir_op, lane1.mir_op);
    emit2 (c2m_ctx, tp_mov (lane_type), lane_val2.mir_op, lane2.mir_op);
    emit3 (c2m_ctx, insn_code, cmp.mir_op, lane_val1.mir_op, lane_val2.mir_op);
    emit3 (c2m_ctx, MIR_SUBS, cmp.mir_op, MIR_new_int_op (ctx, 0), cmp.mir_op);
    mask_val = cast (c2m_ctx, cmp, mask_lane_type, TRUE);
    emit_scalar_assign (c2m_ctx, dest_lane, &mask_val, mask_lane_type, FALSE);
  }
  return dest;
}

static op_t gen_v128_float_cmp_op (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  struct type *type = ((struct expr *) r->attr)->type;
  struct type *type1 = ((struct expr *) NL_HEAD (r->u.ops)->attr)->type;
  struct type *type2 = ((struct expr *) NL_EL (r->u.ops, 1)->attr)->type;
  struct type *vector_type = v128_float_vector_type_p (c2m_ctx, type1) ? type1 : type2;
  op_t op1 = val_gen (c2m_ctx, NL_HEAD (r->u.ops));
  op_t op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
  op_t dest = desirable_dest != NULL ? *desirable_dest : vector_temp (c2m_ctx, type);

  assert (v128_float_vector_type_p (c2m_ctx, vector_type));
...
static op_t gen_convertvector (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  ...
  struct type *from_type = ((struct expr *) value_arg->attr)->type;
...
static op_t emit_shufflevector (c2m_ctx_t c2m_ctx, op_t op1, struct type *type1, op_t op2,
                                struct type *type2, struct type *res_type, node_t first_index,
                                op_t dest) {
  MIR_type_t lane_type = get_mir_type (c2m_ctx, type1->u.vector_type->el_type);
  MIR_type_t lane_reg_type = promoted_v128_lane_reg_type (lane_type);
  mir_size_t lane_size = vector_lane_size (c2m_ctx, type1);
  mir_size_t src1_n_el = vector_el_count (c2m_ctx, type1);
  mir_size_t src2_n_el = vector_el_count (c2m_ctx, type2);
  mir_size_t res_n_el = vector_el_count (c2m_ctx, res_type);
  mir_size_t dest_i = 0;

  assert (shufflevector_type_p (c2m_ctx, res_type, type1, type2, res_n_el));
  op1 = materialize_vector_operand (c2m_ctx, op1, type1);
  op2 = materialize_vector_operand (c2m_ctx, op2, type2);
  for (node_t index_arg = first_index; index_arg != NULL; index_arg = NL_NEXT (index_arg)) {
    struct expr *index_expr = index_arg->attr;
    mir_llong index = signed_integer_type_p (index_expr->type) ? index_expr->c.i_val
                                                               : (mir_llong) index_expr->c.u_val;
    op_t src_lane, dest_lane, lane_val;

    if (index < 0) index = 0;
    assert (dest_i < res_n_el);
    if ((mir_size_t) index < src1_n_el) {
      src_lane = vector_lane_op (op1, lane_type, (mir_size_t) index * lane_size);
    } else {
      mir_size_t src2_i = (mir_size_t) index - src1_n_el;

      assert (src2_i < src2_n_el);
      src_lane = vector_lane_op (op2, lane_type, src2_i * lane_size);
    }
    dest_lane = vector_lane_op (dest, lane_type, dest_i * lane_size);
...
static op_t gen_shufflevector (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  node_t args = NL_EL (r->u.ops, 1);
  node_t arg1 = NL_HEAD (args->u.ops);
  node_t arg2 = NL_NEXT (arg1);
  struct type *type = ((struct expr *) r->attr)->type;
  struct type *type1 = ((struct expr *) arg1->attr)->type;
  struct type *type2 = ((struct expr *) arg2->attr)->type;
  op_t op1 = gen (c2m_ctx, arg1, NULL, NULL, FALSE, NULL, NULL);
  op_t op2 = gen (c2m_ctx, arg2, NULL, NULL, FALSE, NULL, NULL);
  op_t dest = desirable_dest != NULL ? *desirable_dest : vector_temp (c2m_ctx, type);

  return emit_shufflevector (c2m_ctx, op1, type1, op2, type2, type, NL_NEXT (arg2), dest);
}

static op_t vector_indexed_lane_op (op_t vector, MIR_type_t lane_type, mir_size_t lane_size,
                                    op_t index) {
  op_t lane = vector;
  ...
  lane.mir_op.u.mem.alias = lane.mir_op.u.mem.nonalias = 0;
  return lane;
}

static op_t gen_shuffle_index (c2m_ctx_t c2m_ctx, op_t mask, struct type *mask_type,
                               mir_size_t lane_index, mir_size_t modulus) {
  gen_ctx_t gen_ctx = c2m_ctx->gen_ctx;
  MIR_context_t ctx = c2m_ctx->ctx;
  ...
  op_t mask_lane = vector_lane_op (mask, mask_lane_type, lane_index * mask_lane_size);
  ...
  emit3 (c2m_ctx, signed_p ? MIR_MOD : MIR_UMOD, index.mir_op, index.mir_op,
         modulus_op.mir_op);
  if (signed_p) {
    MIR_label_t normalized_label = MIR_new_label (ctx);

    emit3 (c2m_ctx, MIR_BGE, MIR_new_label_op (ctx, normalized_label), index.mir_op,
           zero_op.mir_op);
    emit3 (c2m_ctx, MIR_ADD, index.mir_op, index.mir_op, modulus_op.mir_op);
    emit_label_insn_opt (c2m_ctx, normalized_label);
  }
  return index;
}

static void emit_shuffle_load_lane (c2m_ctx_t c2m_ctx, op_t src, MIR_type_t lane_type,
                                    mir_size_t lane_size, op_t index, op_t dest_lane) {
  op_t src_lane = vector_indexed_lane_op (src, lane_type, lane_size, index);
  MIR_type_t lane_reg_type = promoted_v128_lane_reg_type (lane_type);
...
static op_t emit_shuffle (c2m_ctx_t c2m_ctx, op_t op1, struct type *type1, op_t op2,
                          struct type *type2, op_t mask, struct type *mask_type, op_t dest) {
  MIR_context_t ctx = c2m_ctx->ctx;
  MIR_type_t lane_type = get_mir_type (c2m_ctx, type1->u.vector_type->el_type);
  mir_size_t lane_size = vector_lane_size (c2m_ctx, type1);
  ...
  assert (shuffle_type_p (c2m_ctx, type1, type2, mask_type));
  op1 = materialize_vector_operand (c2m_ctx, op1, type1);
  if (two_source_p) op2 = materialize_vector_operand (c2m_ctx, op2, type2);
  mask = materialize_vector_operand (c2m_ctx, mask, mask_type);
  for (mir_size_t dest_i = 0; dest_i < n_el; dest_i++) {
    op_t index = gen_shuffle_index (c2m_ctx, mask, mask_type, dest_i,
                                    two_source_p ? 2 * n_el : n_el);
    op_t dest_lane = vector_lane_op (dest, lane_type, dest_i * lane_size);

    if (!two_source_p) {
      emit_shuffle_load_lane (c2m_ctx, op1, lane_type, lane_size, index, dest_lane);
    } else {
      MIR_label_t second_label = MIR_new_label (ctx), end_label = MIR_new_label (ctx);
      op_t n_el_op = new_op (NULL, unsigned_mask_p ? MIR_new_uint_op (ctx, n_el)
                                                   : MIR_new_int_op (ctx, n_el));

      emit3 (c2m_ctx, unsigned_mask_p ? MIR_UBGE : MIR_BGE, MIR_new_label_op (ctx, second_label),
             index.mir_op, n_el_op.mir_op);
      emit_shuffle_load_lane (c2m_ctx, op1, lane_type, lane_size, index, dest_lane);
      emit1 (c2m_ctx, MIR_JMP, MIR_new_label_op (ctx, end_label));
      emit_label_insn_opt (c2m_ctx, second_label);
      emit3 (c2m_ctx, MIR_SUB, index.mir_op, index.mir_op, n_el_op.mir_op);
      emit_shuffle_load_lane (c2m_ctx, op2, lane_type, lane_size, index, dest_lane);
      emit_label_insn_opt (c2m_ctx, end_label);
    }
  }
  return dest;
...
static op_t gen_shuffle (c2m_ctx_t c2m_ctx, node_t r, op_t *desirable_dest) {
  node_t args = NL_EL (r->u.ops, 1);
  node_t arg1 = NL_HEAD (args->u.ops);
  node_t arg2 = NL_NEXT (arg1);
  node_t mask_arg = NL_NEXT (arg2) == NULL ? arg2 : NL_NEXT (arg2);
  struct type *type = ((struct expr *) r->attr)->type;
  struct type *type1 = ((struct expr *) arg1->attr)->type;
  struct type *type2 = mask_arg == arg2 ? NULL : ((struct expr *) arg2->attr)->type;
  struct type *mask_type = ((struct expr *) mask_arg->attr)->type;
  op_t op1 = gen (c2m_ctx, arg1, NULL, NULL, FALSE, NULL, NULL);
  op_t op2 = op1;
  op_t mask;
  op_t dest = desirable_dest != NULL ? *desirable_dest : vector_temp (c2m_ctx, type);

  if (type2 != NULL) op2 = gen (c2m_ctx, arg2, NULL, NULL, FALSE, NULL, NULL);
  mask = gen (c2m_ctx, mask_arg, NULL, NULL, FALSE, NULL, NULL);
  return emit_shuffle (c2m_ctx, op1, type1, op2, type2, mask, mask_type, dest);
}
...
static void gen_builtin_abort (c2m_ctx_t c2m_ctx) {
  gen_ctx_t gen_ctx = c2m_ctx->gen_ctx;
  MIR_context_t ctx = c2m_ctx->ctx;
  MIR_op_t args[2];
  MIR_module_t module;
  ...
  if (abort_item == NULL) {
    ...
    abort_proto = MIR_new_proto_arr (ctx, "abort_p", 0, NULL, 0, NULL);
    abort_item = MIR_new_import (ctx, ABORT);
    move_item_to_module_start (module, abort_proto);
    move_item_to_module_start (module, abort_item);
  }
  args[0] = MIR_new_ref_op (ctx, abort_proto);
  args[1] = MIR_new_ref_op (ctx, abort_item);
...
static void gen_initializer (c2m_ctx_t c2m_ctx, size_t init_start, op_t var,
  ...
  if (var.mir_op.mode == MIR_OP_REG) { /* scalar initialization: */
    ...
    if (int128_type_p (((struct expr *) init_el.init->attr)->type)
      ...
      val = int128_mem_to_scalar (c2m_ctx, val, get_mir_type (c2m_ctx, init_el.el_type),
                                  signed_integer_type_p (
                                    ((struct expr *) init_el.init->attr)->type));
    ...
    for (size_t i = init_start; i < VARR_LENGTH (init_el_t, init_els); i++) {
      ...
      if (t != MIR_T_UNDEF && int128_type_p (((struct expr *) init_el.init->attr)->type)
        ...
        val = int128_mem_to_scalar (c2m_ctx, val, t,
                                    signed_integer_type_p (
                                      ((struct expr *) init_el.init->attr)->type));
...
static op_t gen (c2m_ctx_t c2m_ctx, node_t r, MIR_label_t true_label, MIR_label_t false_label,
  ...
  case N_OROR:
    ...
    break;
  case N_BITWISE_NOT:
    if (int128_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
      res = gen_int128_vector_unary_op (c2m_ctx, r, desirable_dest);
      break;
    }
    if (int128_type_p (((struct expr *) r->attr)->type)) {
      res = gen_int128_scalar_unary_op (c2m_ctx, r, NULL);
      break;
    }
    if (supported_integer_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
    ...
    if (complex_type_p (((struct expr *) r->attr)->type)) {
      ...
      enum basic_type cbt = ctype->u.basic_type;
      ...
      int cimag = cbt == TP_CFLOAT ? sizeof (mir_float) : cbt == TP_CDOUBLE ? sizeof (mir_double)
                                                                              : sizeof (mir_ldouble);
      MIR_insn_code_t cneg = cct == MIR_T_F ? MIR_FNEG : cct == MIR_T_D ? MIR_DNEG : MIR_LDNEG;
      op_t im;

      op1 = gen (c2m_ctx, NL_HEAD (r->u.ops), NULL, NULL, FALSE, NULL, NULL);
      res = desirable_dest != NULL ? *desirable_dest : op1;
      if (!MIR_op_eq_p (ctx, res.mir_op, op1.mir_op))
        block_move (c2m_ctx, res, op1, type_size (c2m_ctx, ctype));
      im = complex_load (c2m_ctx, res, cct, cimag);
      emit2 (c2m_ctx, cneg, im.mir_op, im.mir_op);
      complex_store (c2m_ctx, res, cct, cimag, im);
      break;
    }
  ...
  case N_IMAGPART: {
    ...
    MIR_type_t rct = get_mir_type (c2m_ctx, rt);

    op1 = gen (c2m_ctx, NL_HEAD (r->u.ops), NULL, NULL, FALSE, NULL, NULL);
    if (!complex_type_p (((struct expr *) NL_HEAD (r->u.ops)->attr)->type)) {
      /* __real__/__imag__ on non-complex: identity / zero */
      if (r->code == N_REALPART) {
        res = op1;
      } else {
        res = get_new_temp (c2m_ctx, rct);
        emit2 (c2m_ctx, tp_mov (rct), res.mir_op,
               rct == MIR_T_F ? MIR_new_float_op (ctx, 0.0f)
               : rct == MIR_T_D ? MIR_new_double_op (ctx, 0.0)
               : MIR_new_ldouble_op (ctx, 0.0L));
      }
    } else {
      /* Extract component from complex MEM operand */
      struct type *src_type = ((struct expr *) NL_HEAD (r->u.ops)->attr)->type;
      int imoff = src_type->u.basic_type == TP_CFLOAT ? sizeof (mir_float)
                  : src_type->u.basic_type == TP_CDOUBLE ? sizeof (mir_double)
                                                          : sizeof (mir_ldouble);
  ...
  case N_SUB:
    if (NL_NEXT (NL_HEAD (r->u.ops)) == NULL) { /* unary */
      ...
      if (v128_float_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
        res = gen_v128_float_unary_op (c2m_ctx, r, desirable_dest);
        break;
      }
      if (complex_type_p (((struct expr *) r->attr)->type)) {
  ...
  case N_MOD:
    ...
    if ((r->code == N_LSH || r->code == N_RSH)
        && int128_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
      res = gen_int128_vector_shift_op (c2m_ctx, r, desirable_dest);
      break;
    }
    if ((r->code == N_ADD || r->code == N_SUB || r->code == N_AND || r->code == N_OR
         || r->code == N_XOR || r->code == N_MUL || r->code == N_DIV || r->code == N_MOD)
        && int128_type_p (((struct expr *) r->attr)->type)) {
      res = gen_int128_scalar_bin_op (c2m_ctx, r, NULL);
      break;
    }
    if ((r->code == N_LSH || r->code == N_RSH)
        && int128_type_p (((struct expr *) r->attr)->type)) {
      res = gen_int128_scalar_shift_op (c2m_ctx, r, NULL);
      break;
    }
    if ((r->code == N_ADD || r->code == N_SUB || r->code == N_MUL || r->code == N_DIV)
        && v128_float_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
      res = gen_v128_float_arith_op (c2m_ctx, r, desirable_dest);
      break;
    }
    if ((r->code == N_LSH || r->code == N_RSH)
        && supported_integer_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
      res = gen_v128_i32_shift_op (c2m_ctx, r, desirable_dest);
      break;
    }
    if ((r->code == N_MUL || r->code == N_DIV || r->code == N_MOD)
        && supported_integer_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
      res = gen_v128_i32_arith_op (c2m_ctx, r, desirable_dest);
      break;
    }
  ...
  case N_GE: {
    ...
    if (v128_float_cmp_vector_type (c2m_ctx, type1, type2) != NULL) {
      res = gen_v128_float_cmp_op (c2m_ctx, r, desirable_dest);
      break;
    }
    if (int128_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
      res = gen_int128_vector_cmp_op (c2m_ctx, r, desirable_dest);
      break;
    }
    if (supported_integer_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type)) {
    ...
    if (int128_type_p (&type_s)) {
      res = gen_int128_scalar_cmp_op (c2m_ctx, r, &type_s, true_label, false_label);
      if (true_label != NULL) true_label = false_label = NULL;
      break;
    }
  ...
  case N_POST_DEC: {
    ...
    if (int128_vector_type_p (c2m_ctx, type)
        || supported_integer_vector_type_p (c2m_ctx, type)
    ...
    if (int128_type_p (type)) {
      struct type *vtype = int128_one_lane_vector_type (c2m_ctx, type, r);
      op_t one128 = const_int128_vector (c2m_ctx, vtype, 1, 0);

      var = gen (c2m_ctx, NL_HEAD (r->u.ops), NULL, NULL, FALSE, NULL, NULL);
      res = var;
      if (val_p || true_label != NULL) { /* the value is the OLD one */
        res = int128_temp (c2m_ctx);
        block_move (c2m_ctx, res, var, 16);
      }
      emit_int128_vector_bin_op (c2m_ctx, r->code == N_POST_INC ? N_ADD : N_SUB, var, type,
                                 one128, type, vtype, var);
      break;
    }
  ...
  case N_DEC: {
    ...
    if (int128_type_p (type)) {
      ...
      var = gen (c2m_ctx, NL_HEAD (r->u.ops), NULL, NULL, FALSE, NULL, NULL);
      emit_int128_vector_bin_op (c2m_ctx, r->code == N_INC ? N_ADD : N_SUB, var, type, one128,
                                 type, vtype, var);
      res = var; /* the value is the NEW one */
      break;
    }
    t = get_mir_type (c2m_ctx, type);
    var = gen (c2m_ctx, NL_HEAD (r->u.ops), NULL, NULL, FALSE, NULL, NULL);
  ...
  case N_MOD_ASSIGN:
    if ((r->code == N_ADD_ASSIGN || r->code == N_SUB_ASSIGN || r->code == N_MUL_ASSIGN
      ...
      op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
      res = emit_int128_vector_bin_op (c2m_ctx, r->code, val, lhs_type, op2, rhs_type,
                                       lhs_type, var);
      break;
    }
    if ((r->code == N_LSH_ASSIGN || r->code == N_RSH_ASSIGN)
      ...
      val = force_val (c2m_ctx, var, FALSE);
      op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
      res = emit_int128_vector_shift_op (c2m_ctx, r->code, val, lhs_type, op2, rhs_type,
                                         lhs_type, var);
      break;
    }
    if ((r->code == N_ADD_ASSIGN || r->code == N_SUB_ASSIGN || r->code == N_MUL_ASSIGN
         || r->code == N_DIV_ASSIGN || r->code == N_MOD_ASSIGN || r->code == N_AND_ASSIGN
         || r->code == N_OR_ASSIGN || r->code == N_XOR_ASSIGN)
        && int128_type_p (((struct expr *) r->attr)->type2)) {
      /* Scalar __int128 compound assignment via the one-lane-vector emitters. */
      struct type *lhs_type = ((struct expr *) r->attr)->type2;
      struct type *rhs_type = ((struct expr *) NL_EL (r->u.ops, 1)->attr)->type;
      struct type *vtype = int128_one_lane_vector_type (c2m_ctx, lhs_type, r);

      var = gen (c2m_ctx, NL_HEAD (r->u.ops), NULL, NULL, FALSE, NULL, NULL);
      op2 = materialize_int128_vector_node (c2m_ctx, NL_EL (r->u.ops, 1), rhs_type);
      res = emit_int128_vector_bin_op (c2m_ctx, r->code, var, lhs_type, op2, lhs_type, vtype,
                                       var);
      break;
    }
    if ((r->code == N_LSH_ASSIGN || r->code == N_RSH_ASSIGN)
      ...
      var = gen (c2m_ctx, NL_HEAD (r->u.ops), NULL, NULL, FALSE, NULL, NULL);
      op2 = int128_type_p (rhs_type)
              ? materialize_int128_scalar_node (c2m_ctx, NL_EL (r->u.ops, 1), rhs_type)
              : val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
      res = emit_int128_vector_shift_op (c2m_ctx, r->code, var, lhs_type, op2, rhs_type, vtype,
                                         var);
      break;
    }
    if ((r->code == N_ADD_ASSIGN || r->code == N_SUB_ASSIGN || r->code == N_MUL_ASSIGN
         || r->code == N_DIV_ASSIGN)
        && v128_float_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type2)) {
      struct type *lhs_type = ((struct expr *) r->attr)->type2;
      struct type *rhs_type = ((struct expr *) NL_EL (r->u.ops, 1)->attr)->type;
      ...
      val = force_val (c2m_ctx, var, FALSE);
      op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
      res = emit_v128_float_arith_op (c2m_ctx, r->code, val, lhs_type, op2, rhs_type, lhs_type,
                                      var);
      break;
    }
    if ((r->code == N_LSH_ASSIGN || r->code == N_RSH_ASSIGN)
        && supported_integer_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type2)) {
      struct type *lhs_type = ((struct expr *) r->attr)->type2;
      struct type *rhs_type = ((struct expr *) NL_EL (r->u.ops, 1)->attr)->type;
      ...
      val = force_val (c2m_ctx, var, FALSE);
      op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
      res = emit_v128_i32_shift_op (c2m_ctx, r->code, val, lhs_type, op2, rhs_type, lhs_type, var);
      break;
    }
    if ((r->code == N_MUL_ASSIGN || r->code == N_DIV_ASSIGN || r->code == N_MOD_ASSIGN)
        && supported_integer_vector_type_p (c2m_ctx, ((struct expr *) r->attr)->type2)) {
      struct type *lhs_type = ((struct expr *) r->attr)->type2;
      struct type *rhs_type = ((struct expr *) NL_EL (r->u.ops, 1)->attr)->type;
      ...
      val = force_val (c2m_ctx, var, FALSE);
      op2 = val_gen (c2m_ctx, NL_EL (r->u.ops, 1));
      res = emit_v128_i32_arith_op (c2m_ctx, r->code, val, lhs_type, op2, rhs_type, lhs_type, var);
      break;
    }
    insn_code = get_v128_i32_bin_insn_code (r->code);
    if (insn_code != MIR_INSN_BOUND
      ...
      if (r->code == N_ADD_ASSIGN || r->code == N_SUB_ASSIGN) {
        if (v128_i32_packed_add_sub_vector_type_p (c2m_ctx, lhs_type)) {
          insn_code = get_v128_i32_packed_add_sub_insn_code (c2m_ctx, r->code, lhs_type);
          res
            = emit_v128_i32_bin_op (c2m_ctx, insn_code, val, lhs_type, op2, rhs_type, lhs_type,
                                    var);
        } else {
          res = emit_v128_i32_arith_op (c2m_ctx, r->code, val, lhs_type, op2, rhs_type,
                                        lhs_type, var);
        }
      } else {
        if (v128_i32_vector_type_p (c2m_ctx, lhs_type)) {
          res = emit_v128_i32_bin_op (c2m_ctx, insn_code, val, lhs_type, op2, rhs_type, lhs_type,
                                      var);
        } else {
          res = emit_v128_i32_lane_bin_op (
            c2m_ctx, get_integer_vector_bitwise_insn_code (c2m_ctx, r->code, lhs_type), val,
            lhs_type, op2, rhs_type, lhs_type, var);
        }
      }
      break;
    }
  ...
  case N_ASSIGN:
    var = gen (c2m_ctx, NL_HEAD (r->u.ops), NULL, NULL, FALSE, NULL, NULL);
    if (int128_type_p (((struct expr *) r->attr)->type)) {
      /* Scalar __int128 assignment: materialize the rhs at the node level
         (const-correct, converts narrower sources) and block-move 16 bytes. */
      struct type *rtype = ((struct expr *) NL_EL (r->u.ops, 1)->attr)->type;

      val = materialize_int128_scalar_node (c2m_ctx, NL_EL (r->u.ops, 1), rtype);
      block_move (c2m_ctx, var, val, 16);
      res = var;
      break;
    }
    t = get_op_type (c2m_ctx, var);
    ...
    if (t != MIR_T_UNDEF && int128_type_p (((struct expr *) NL_EL (r->u.ops, 1)->attr)->type)
      ...
      op2 = int128_mem_to_scalar (c2m_ctx, op2,
                                  t == MIR_T_F || t == MIR_T_D || t == MIR_T_LD
                                    ? t
                                    : promote_mir_int_type (t),
                                  signed_integer_type_p (
  ...
  case N_CAST: {
    ...
    } else if (vector_type_p (type) && same_size_vector_integer_cast_p (c2m_ctx, type, from_type)) {
      MIR_type_t scalar_type = get_mir_type (c2m_ctx, from_type);
      op_t dest;

      res = desirable_dest != NULL ? *desirable_dest : vector_temp (c2m_ctx, type);
      dest = res;
      assert (dest.mir_op.mode == MIR_OP_MEM);
      dest.mir_op.u.mem.type = scalar_type;
      op1 = cast (c2m_ctx, op1, scalar_type, TRUE);
      emit_scalar_assign (c2m_ctx, dest, &op1, scalar_type, FALSE);
    } else if (integer_type_p (type)
               && same_size_vector_integer_cast_p (c2m_ctx, type, from_type)) {
      t = get_mir_type (c2m_ctx, type);
      op1 = materialize_vector_operand (c2m_ctx, op1, from_type);
      op1.mir_op.u.mem.type = t;
    ...
    } else if (int128_type_p (from_type) && int128_type_p (type)) {
      /* int128 <-> uint128: same bits, same storage. */
      res = op1;
    } else if (int128_type_p (from_type)) {
      /* Scalar __int128 -> narrower scalar.  (The cast operand of a
         non-const cast is non-const, so op1 is already the 16-byte memory
         value.) */
      assert (op1.mir_op.mode == MIR_OP_MEM);
      if (void_type_p (type)) {
        res = op1;
      } else if (type->mode == TM_BASIC && type->u.basic_type == TP_BOOL) {
        res = int128_scalar_truth_val (c2m_ctx, op1); /* (bool)x is x != 0, not the low half */
      } else {
        assert (integer_type_p (type) || floating_type_p (type) || type->mode == TM_PTR);
        t = get_mir_type (c2m_ctx, type);
        res = int128_mem_to_scalar (c2m_ctx, op1, t, signed_integer_type_p (from_type));
      }
    } else if (int128_type_p (type)) {
      /* Scalar -> __int128 in a 16-byte temp.  A pointer source widens as an
         unsigned 64-bit value; an explicit floating cast picks the helper
         variant from the TARGET's signedness. */
      assert (integer_type_p (from_type) || floating_type_p (from_type)
              || from_type->mode == TM_PTR);
      if (floating_type_p (from_type)) {
        res = float_to_int128_mem (c2m_ctx, op1, get_mir_type (c2m_ctx, from_type),
                                   signed_integer_type_p (type));
      } else if (from_type->mode == TM_PTR) {
        struct type ptr_int_type;

        init_type (&ptr_int_type);
        ptr_int_type.mode = TM_BASIC;
        ptr_int_type.u.basic_type = TP_ULONG;
        res = scalar_to_int128_mem (c2m_ctx, op1, &ptr_int_type);
      } else {
        res = scalar_to_int128_mem (c2m_ctx, op1, from_type);
      }
    } else {
      t = get_mir_type (c2m_ctx, type);
      res = cast (c2m_ctx, op1, t, TRUE);
  ...
  case N_CALL: {
    ...
    if (shuffle_p) {
      res = gen_shuffle (c2m_ctx, r, desirable_dest);
      break;
    }
    if (shufflevector_p) {
      res = gen_shufflevector (c2m_ctx, r, desirable_dest);
      break;
    }
    if (add_overflow_p || sub_overflow_p || mul_overflow_p) {
      ...
      if (int128_type_p (e->type->u.ptr_type)) {
        /* int128-result overflow builtins lower to helper calls (the host
           builtin covers the INT128_MIN / -1 edge cases).  Operands convert
           value-preservingly to 128 bits; the overflow flag follows the
           result type's signedness (same approximation as the native-insn
           path below, which also computes at the result type). */
        static const char *const onames[6]
          = {"__mir_addoti", "__mir_uaddoti", "__mir_suboti",
             "__mir_usuboti", "__mir_muloti", "__mir_umuloti"};
        int signed_p = signed_integer_type_p (e->type->u.ptr_type);
        int oi = (add_overflow_p ? 0 : sub_overflow_p ? 2 : 4) + !signed_p;
        struct type *at;
        op_t srcs[2], low1, high1, low2, high2, flag;
        MIR_op_t cargs[8];

        for (n = 0; n < 2; n++) {
          op_t aop = n == 0 ? op1 : op2;

          at = ((struct expr *) NL_EL (args->u.ops, n)->attr)->type;
          if (!int128_type_p (at))
        ...
        if (int128_ovfl_proto == NULL) {
          MIR_type_t ovfl_ret = MIR_T_I64;
          MIR_var_t ovfl_vars[5];

          ovfl_vars[0].name = "res_addr";
          ovfl_vars[0].type = MIR_T_I64;
          ovfl_vars[1].name = "a_low";
          ovfl_vars[1].type = MIR_T_U64;
          ovfl_vars[2].name = "a_high";
          ovfl_vars[2].type = MIR_T_U64;
          ovfl_vars[3].name = "b_low";
          ovfl_vars[3].type = MIR_T_U64;
          ovfl_vars[4].name = "b_high";
          ovfl_vars[4].type = MIR_T_U64;
          int128_ovfl_proto = MIR_new_proto_arr (ctx, "i128_ovfl_p", 1, &ovfl_ret, 5, ovfl_vars);
          move_item_to_module_start (curr_func->module, int128_ovfl_proto);
        }
        if (int128_ovfl_items[oi] == NULL) {
          void *addr = MIR_int128_helper_resolver (onames[oi]);

          int128_ovfl_items[oi] = MIR_new_import (ctx, onames[oi]);
          if (addr != NULL) MIR_load_external (ctx, onames[oi], addr);
          move_item_to_module_start (curr_func->module, int128_ovfl_items[oi]);
        }
        load_int128_halves (c2m_ctx, srcs[0], MIR_T_U64, &low1, &high1);
        load_int128_halves (c2m_ctx, srcs[1], MIR_T_U64, &low2, &high2);
        flag = get_new_temp (c2m_ctx, MIR_T_I64);
        cargs[0] = MIR_new_ref_op (ctx, int128_ovfl_proto);
        cargs[1] = MIR_new_ref_op (ctx, int128_ovfl_items[oi]);
        cargs[2] = flag.mir_op;
        cargs[3] = op3.mir_op;
        cargs[4] = low1.mir_op;
        cargs[5] = high1.mir_op;
        cargs[6] = low2.mir_op;
        cargs[7] = high2.mir_op;
        emit_insn (c2m_ctx, MIR_new_insn_arr (ctx, MIR_CALL, 8, cargs));
        if (true_label != NULL) {
    ...
    if (va_arg_p) {
      ...
      for (node_t arg = first_arg; arg != NULL; arg = NL_NEXT (arg)) {
        ...
        if (int128_type_p (arg_type)) {
          /* Scalar __int128 argument: node-level materialization is
             const-correct and converts a narrower integer argument to the
             parameter's 128-bit shape (structs never need this — their
             argument type always matches). */
          op2 = materialize_int128_vector_node (c2m_ctx, arg, e->type);
        ...
        if (!memory_arg_p && int128_type_p (e->type) && op2.mir_op.mode == MIR_OP_MEM) {
          ...
          MIR_type_t at = get_mir_type (c2m_ctx, arg_type);

          op2 = int128_mem_to_scalar (c2m_ctx, op2,
                                      at == MIR_T_F || at == MIR_T_D || at == MIR_T_LD
                                        ? at
                                        : MIR_T_I64,
  ...
  case N_SWITCH: {
    ...
    signed_p = signed_integer_type_p (type);
    if (int128_type_p (type)) {
      /* switch on a scalar __int128: a halves compare chain (no MIR_SWITCH
         table — case values may not fit 64 bits). */
      op_t low, high;
      op_t src = case_reg_op.mir_op.mode == MIR_OP_MEM
                   ? case_reg_op
                   : materialize_int128_scalar_node (c2m_ctx, expr, type);

      load_int128_halves (c2m_ctx, src, MIR_T_U64, &low, &high);
      for (c = DLIST_HEAD (case_t, switch_attr->case_labels); c != NULL;
        ...
        if (case_expr2 == NULL) {
          ...
          emit3 (c2m_ctx, MIR_BNE, MIR_new_label_op (ctx, skip), high.mir_op,
                 MIR_new_uint_op (ctx, e->c_u_hi_val));
          emit3 (c2m_ctx, MIR_BEQ, MIR_new_label_op (ctx, label), low.mir_op,
                 MIR_new_uint_op (ctx, (mir_ullong) e->c.i_val));
          emit_label_insn_opt (c2m_ctx, skip);
        } else {
          e2 = case_expr2->attr;
          assert (e2->const_p && integer_type_p (e2->type));
          cont_label = MIR_new_label (ctx);
          /* v < L -> next case; v <= H -> this case's label */
          emit_int128_branch_lt_le (c2m_ctx, cont_label, FALSE, signed_p, low, high,
                                    (mir_ullong) e->c.i_val, e->c_u_hi_val);
          emit_int128_branch_lt_le (c2m_ctx, label, TRUE, signed_p, low, high,
                                    (mir_ullong) e2->c.i_val, e2->c_u_hi_val);
  ...
  case N_RETURN: {
    ...
    if (!ret_by_addr_p && int128_type_p (ret_type)) {
      /* Scalar __int128 return value: node-level materialization is
         const-correct and widens a narrower returned value. */
      ...
      if (scalar_return_p) {
        if (int128_type_p (((struct expr *) NL_EL (r->u.ops, 1)->attr)->type)
          ...
          MIR_type_t rt = get_mir_type (c2m_ctx, ret_type);

          val = int128_mem_to_scalar (c2m_ctx, val,
                                      rt == MIR_T_F || rt == MIR_T_D || rt == MIR_T_LD
                                        ? rt
                                        : MIR_T_I64,
                                      signed_integer_type_p (
                                        ((struct expr *) NL_EL (r->u.ops, 1)->attr)->type));
...
finish:
  if (true_label != NULL) {
    ...
    if (complex_type_p (type)) {
      ...
      emit1 (c2m_ctx, MIR_JMP, MIR_new_label_op (ctx, false_label));
    } else if (int128_type_p (type) && res.mir_op.mode == MIR_OP_MEM) {
      /* Scalar __int128 truth test: nonzero in either half. */
      res = int128_scalar_truth_val (c2m_ctx, res);
